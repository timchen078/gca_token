...
class GcaWorkerDeployReadinessTests(unittest.TestCase):
    def setUp(self):
        ...
        (worker_dir / "migrations" / "0012_service_request_reviews.sql").write_text(
        ...
        (worker_dir / "migrations" / "0014_service_request_cancellations.sql").write_text(
            "ALTER TABLE gca_service_requests ADD COLUMN cancellation_id TEXT;\n",
            encoding="utf-8",
        )
        (worker_dir / "migrations" / "0015_service_request_followups.sql").write_text(
            "ALTER TABLE gca_service_request_reviews ADD COLUMN member_prompt TEXT;\n",
            encoding="utf-8",
        )
