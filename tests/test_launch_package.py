...
class LaunchPackageTests(unittest.TestCase):
    ...
    def test_status_page_centralizes_public_verification(self):
        ...
        self.assertIn("Returned 2026-05-23", status)
        self.assertIn("BaseScan Handoff, Chinese owner flow, Tim Chen profile, and domain email evidence are ready for one owner resubmission from support@gcagochina.com", status)
        self.assertIn("BaseScan Handoff", status)
        self.assertIn("中文 BaseScan 提交流程", status)
        self.assertIn("no wallet transaction, approve, swap, or contract operation needed", status)
        self.assertIn("2026-05-30 DNS snapshot: MX/SPF/DKIM/DMARC present", status)
        self.assertIn("BaseScan remediation DNS gate", status)
        self.assertIn("ready after domain email DNS, evidence packet, public email switch, and snapshot alignment passed", status)
