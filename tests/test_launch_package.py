...
class LaunchPackageTests(unittest.TestCase):
    ...
    def test_public_site_discloses_current_operational_status(self):
        ...
        self.assertIn("does not activate credits or membership", site)
        self.assertIn("each registered user may verify one wallet holding at least 10,000 GCA", site)
        self.assertIn("one-time 100 Web3 Radar utility credits bonus", site)
    ...
    def test_utility_page_connects_gca_to_quant_tools_safely(self):
        ...
        self.assertIn("Planned Holder Programs", utility)
        self.assertIn("one wallet holding at least 10,000 GCA", utility)
        self.assertIn("one-time 100 Web3 Radar utility credits bonus", utility)
    ...
    def test_data_platform_package_is_copyable_and_conservative(self):
        ...
        self.assertIn("one-time 100 Web3 Radar utility credits bonus for each registered user who verifies one wallet holding at least 10000 GCA after the access bridge is live", values["utilityPositioning"]["suitableUtility"])
        self.assertIn("GCA Member status for each registered user who verifies one wallet holding at least 1000000 GCA after the access bridge is live", values["utilityPositioning"]["suitableUtility"])
