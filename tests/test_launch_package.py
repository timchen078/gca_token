...
TOOLS_PAGE_URL = "https://gcagochina.com/tools.html"
...
class LaunchPackageTests(unittest.TestCase):
    ...
    def test_public_site_health_check_script_matches_canonical_identity(self):
        ...
        module.validate_zh_product_page((ROOT / "site" / "zh-product.html").read_text())
        ...
        module.validate_market_snapshot_script((ROOT / "site" / "assets" / "market-snapshot.js").read_text())
        module.validate_contract_identity_script((ROOT / "site" / "assets" / "contract-identity.js").read_text())
        module.validate_reserve_balance_script((ROOT / "site" / "assets" / "reserve-balance.js").read_text())
    ...
    def test_registration_api_public_check_workflow_runs_without_secrets(self):
        ...
        self.assertIn("python tools/check_gca_registration_api.py --public-only --timeout 30 --include-service-routes", workflow)
        self.assertIn("for attempt in {1..20}", workflow)
        self.assertIn("retrying in 15 seconds", workflow)
        self.assertIn("sleep 15", workflow)
        self.assertNotIn("contents: write", workflow)
    ...
    def test_daily_ops_public_check_workflow_runs_without_member_exports(self):
        ...
        self.assertIn('"tools/sync_basescan_daily_status_references.py"', workflow)
        self.assertIn('"tools/sync_basescan_final_package_references.py"', workflow)
        self.assertIn('"tools/check_public_site.py"', workflow)
        self.assertIn('"tools/check_gca_registration_api.py"', workflow)
        self.assertIn('"launch/basescan_resubmission_values.json"', workflow)
        self.assertIn('"launch/basescan_final_submission_package.json"', workflow)
        self.assertIn('"site/basescan-handoff.json"', workflow)
        ...
        self.assertIn("python -m pip install certifi", workflow)
        self.assertIn("SSL_CERT_FILE", workflow)
        self.assertIn("python tools/sync_basescan_daily_status_references.py --check", workflow)
        self.assertIn("python tools/sync_basescan_final_package_references.py --check", workflow)
        ...
        self.assertIn("--digest-json-output /tmp/gca-public-daily-ops/gca_operator_digest.json", workflow)
        ...
        self.assertIn("/tmp/gca-public-daily-ops/", workflow)
        self.assertIn("MAX_ATTEMPTS=3", workflow)
        self.assertIn('for attempt in $(seq 1 "$MAX_ATTEMPTS")', workflow)
        self.assertIn("gca_daily_ops_summary_attempt_${attempt}.json", workflow)
        self.assertIn("gca_operator_digest_attempt_${attempt}.json", workflow)
        self.assertIn("gca_operator_digest_attempt_${attempt}.md", workflow)
        self.assertIn("failed after ${MAX_ATTEMPTS} attempts", workflow)
        self.assertIn("retrying in 15 seconds", workflow)
        self.assertIn("sleep 15", workflow)
    ...
    def test_daily_status_publish_workflow_is_success_gated_and_public_only(self):
        workflow_path = ROOT / ".github" / "workflows" / "publish-gca-daily-status.yml"
        workflow = workflow_path.read_text()

        self.assertIn("name: Publish GCA Daily Status", workflow)
        self.assertIn("workflow_run:", workflow)
        self.assertIn('      - "Check GCA Daily Ops"', workflow)
        ...
        self.assertIn("contents: write", workflow)
        self.assertIn("group: gca-site-publish", workflow)
        ...
        self.assertNotIn("gca_operator_digest", workflow)
        self.assertNotIn("--include-member-ops", workflow)
        self.assertNotIn("ADMIN_READ_TOKEN", workflow)
        self.assertNotIn("secrets.", workflow)
        self.assertNotIn("mnemonic", workflow.lower())
    ...
    def test_daily_status_page_and_json_publish_public_ops_snapshot(self):
        ...
        self.assertIn("official-pool-market-health", page)
        self.assertIn("GCA/USDT Market Health", page)
        self.assertIn("read-only aggregate market check", page)
        self.assertIn("basescan-public-profile-status", page)
        self.assertIn("basescan-resubmission-preflight-status", page)
        self.assertIn("Public BaseScan Profile", page)
        self.assertIn("Public profile observation", page)
        self.assertIn("Public Token Rep", page)
        ...
        self.assertEqual(daily_steps["public-site"]["command"], "python3 tools/check_public_site.py --base-url https://gcagochina.com/ --timeout 30")
        self.assertEqual(daily_steps["registration-api-public"]["command"], "python3 tools/check_gca_registration_api.py --base-url https://gca-registration-api.gcagochina.workers.dev --public-only --timeout 30")
        self.assertEqual(daily_steps["official-pool-market-health"]["command"], "python3 tools/check_gca_market_health.py --json --timeout 30")
        self.assertFalse(daily_steps["official-pool-market-health"]["blocksSummaryOk"])
        self.assertFalse(daily_steps["basescan-resubmission-preflight-status"]["blocksSummaryOk"])
        self.assertFalse(daily_steps["basescan-public-profile-status"]["blocksSummaryOk"])
        ...
        self.assertEqual(payload["marketHealth"]["status"], "official-pool-observed")
        self.assertTrue(payload["marketHealth"]["available"])
        self.assertTrue(payload["marketHealth"]["identityVerified"])
        self.assertEqual(payload["marketHealth"]["poolAddress"], OFFICIAL_POOL_ADDRESS)
        self.assertEqual(payload["marketHealth"]["contractAddress"], MAINNET_ADDRESS)
        self.assertEqual(payload["marketHealth"]["quoteAssetAddress"].lower(), BASE_USDT_ADDRESS.lower())
        self.assertEqual(payload["marketHealth"]["dexId"], "uniswap-v4-base")
        self.assertTrue(payload["marketHealth"]["readOnly"])
        self.assertTrue(payload["marketHealth"]["aggregateDataOnly"])
        self.assertTrue(payload["marketHealth"]["notExecutableQuote"])
        ...
        self.assertEqual(payload["baseScanPublicProfile"]["tokenRep"], "Unknown")
        self.assertTrue(payload["baseScanPublicProfile"]["sourceVerificationObserved"])
        self.assertFalse(payload["baseScanPublicProfile"]["officialDomainPresent"])
        self.assertTrue(payload["baseScanPublicProfile"]["genericAddressTitle"])
        self.assertTrue(payload["baseScanPublicProfile"]["defaultPreviewImage"])
        self.assertTrue(payload["baseScanPublicProfile"]["readOnly"])
        ...
        self.assertTrue(payload["boundaries"]["readsPublicMarketApiOnly"])
        self.assertFalse(payload["boundaries"]["buildsExecutableMarketQuote"])
    ...
    def test_start_page_guides_normal_users_away_from_raw_json(self):
        ...
        self.assertIn("GCA Quick Start | Verify, Explore and Join", page)
        self.assertIn("GCA Quick Start / GCA 快速开始", page)
        self.assertIn("Verify first. Then explore GCA.", page)
        self.assertIn("Four steps from verification to access.", page)
        self.assertIn("Choose the route that matches your goal.", page)
        self.assertIn("Know what GCA does and does not control.", page)
        self.assertIn("Match every address before interacting.", page)
        ...
        self.assertIn("Membership Rules", page)
        ...
        self.assertIn("members.html", page)
        self.assertIn("zh-members.html", page)
        self.assertNotIn('href="data.html"', page)
        self.assertIn("product.html", page)
        self.assertNotIn("中文会员审核资料清单", page)
        self.assertNotIn("zh-member-checklist.html", page)
        self.assertNotIn("Review Queue", page)
        self.assertNotIn("Release Gates", page)
        self.assertNotIn("member-ledger.html", page)
        self.assertNotIn("member-benefit.html", page)
        self.assertNotIn("member-benefit-transfer.html", page)
        self.assertNotIn('href="operations.html"', page)
        self.assertNotIn('href="operator.html"', page)
    ...
    def test_basescan_remediation_page_and_json_track_return_notice(self):
        ...
        self.assertEqual(final_package["generatedAt"], "2026-08-11T18:56:43Z")
        ...
        self.assertIn("daily status reviewer reference alignment passes", data["nextSubmissionGate"]["preflightTool"]["requires"])
        ...
        self.assertEqual(checklist_builder["baseScanFinalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        self.assertEqual(checklist_builder["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
        ...
        self.assertIn("daily status reviewer reference alignment passes", data["nextSubmissionGate"]["submissionPackageBuilder"]["readyRequires"])
    ...
    def test_basescan_reviewer_checklist_artifacts_are_current_and_blocked(self):
        ...
        self.assertEqual(data["baseScanFinalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        self.assertEqual(data["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
    ...
    def test_basescan_preflight_page_and_json_gate_next_resubmission(self):
        ...
        self.assertIn("tools/sync_basescan_daily_status_references.py --check --json", page)
        self.assertIn("tools/sync_basescan_final_package_references.py --check --json", page)
        self.assertIn("tools/check_basescan_resubmission_readiness.py", page)
        self.assertIn("tools/build_basescan_submission_package.py", page)
        ...
        self.assertEqual(data["lastUpdated"], "2026-08-12")
        ...
        self.assertEqual(data["preflightRefresh"]["finalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        ...
        self.assertEqual(data["preflightRefresh"]["dailyStatusSnapshotRefreshedAt"], "2026-08-12T06:37:31Z")
        ...
        self.assertIn("tools/sync_basescan_daily_status_references.py --check --json", data["commands"]["dailyStatusReferenceAlignment"])
    ...
    def test_basescan_handoff_page_and_json_index_resubmission_evidence(self):
        ...
        self.assertIn("tools/sync_basescan_final_package_references.py --check", page)
        ...
        self.assertIn("ten browser-only public risk and research tools", page)
        ...
        self.assertEqual(data["finalSubmissionPackage"]["generatedAt"], "2026-08-11T18:56:43Z")
        self.assertEqual(data["finalSubmissionPackage"]["referenceSynchronization"]["status"], "aligned-after-canonical-build")
        ...
        self.assertIn("Confirm tools/sync_basescan_final_package_references.py --check reports aligned with zero changed files.", data["submissionSequence"])
    ...
    def test_basescan_followup_page_and_json_prepare_no_reply_ticket_reply(self):
        ...
        self.assertIn("Latest Public Evidence", page)
        self.assertIn("2026-06-15 Status Refresh", page)
        self.assertIn("public website check passing", page)
        self.assertIn("public registration API check passing", page)
        ...
        self.assertEqual(data["lastUpdated"], "2026-06-15")
        self.assertEqual(data["chainId"], 8453)
        self.assertEqual(data["contractAddress"], MAINNET_ADDRESS)
        ...
        self.assertIn("2026-08-12T06:37:31Z", data["followupTemplate"]["body"])
        self.assertIn("avoid duplicate token-profile submissions", data["followupTemplate"]["body"])
        self.assertEqual(data["latestPublicEvidence"]["snapshotGeneratedAt"], "2026-08-12T06:37:31Z")
        self.assertEqual(data["latestPublicEvidence"]["dailyStatusPage"], DAILY_STATUS_PAGE_URL)
        self.assertEqual(data["latestPublicEvidence"]["dailyStatusJson"], DAILY_STATUS_URL)
        self.assertTrue(data["latestPublicEvidence"]["readyForBaseScanResubmission"])
        self.assertEqual(data["latestPublicEvidence"]["currentPublicEmail"], "support@gcagochina.com")
        self.assertEqual(data["latestPublicEvidence"]["filesStillUsingOldEmail"], 0)
        self.assertEqual(data["latestPublicEvidence"]["filesPublishingForbiddenLegacyEmail"], 0)
        for key in (
            "submitsBaseScanRequest",
            "sendsEmail",
            "uploadsFiles",
    ...
    def test_operator_console_reads_local_ledgers_without_public_claims(self):
        ...
        self.assertIn("Service requests", page)
        self.assertIn("Pending service review", page)
        self.assertIn("Waiting member info", page)
        self.assertIn("Approved / delivery due", page)
        self.assertIn("Delivered services", page)
        self.assertIn("Service follow-ups", page)
        self.assertIn("digestServiceRequests", page)
        self.assertIn("digestServicePending", page)
        self.assertIn("digestServiceNeedsInfo", page)
        self.assertIn("digestServiceAwaitingDelivery", page)
        self.assertIn("digestServiceDelivered", page)
        self.assertIn("digestServiceFollowups", page)
        self.assertIn("serviceMetricsAvailable", page)
        self.assertIn("serviceRequestsPendingReview", page)
        self.assertIn("serviceRequestsAwaitingMemberFollowup", page)
        self.assertIn("serviceRequestsAwaitingDelivery", page)
        self.assertIn("deliveredServiceRequests", page)
        ...
        self.assertIn("Export Review Chain Checkpoint", page)
        self.assertIn('id="exportCheckpointButton" type="button" disabled', page)
        ...
        self.assertIn("Support review records", page)
        self.assertIn("Review trail integrity", page)
        self.assertIn("Chained review events", page)
        self.assertIn("Review chain head", page)
        self.assertIn("tools/verify_gca_support_review_audit.py", page)
        self.assertIn("tools/create_gca_support_review_checkpoint.py", page)
        self.assertIn("--checkpoint PATH_TO_RETAINED_CHECKPOINT.json", page)
        self.assertIn("REVIEW_CHECKPOINT_ENDPOINT_PATH", page)
        self.assertIn("exportReviewCheckpoint", page)
        self.assertIn("Support review chain checkpoint exported", page)
        self.assertIn("No independently retained checkpoint exported in this browser session", page)
        self.assertIn("not a digital signature, external timestamp, immutable database", page)
        self.assertIn("cannot prove that the tail was not truncated", page)
        ...
        self.assertIn("Audit Seq", page)
        self.assertIn("Audit Hash", page)
        self.assertIn('data-review-id="', page)
        self.assertNotIn("data-review-index", page)
    ...
    def test_member_access_page_is_live_and_safe(self):
        ...
        self.assertIn("/gca/member-access", page)
        self.assertIn("/gca/wallet-verifications", page)
        self.assertIn("gca_member_access_v2", page)
        self.assertIn("gca_account_status_v1", page)
        self.assertIn("/gca/account-status", page)
        self.assertIn("Refresh Account Status", page)
        self.assertIn("Forget This Device", page)
    ...
    def test_support_page_and_json_define_safe_intake(self):
        ...
        self.assertIn("GCA Support", page)
        self.assertIn("How can we help?", page)
        self.assertIn("Help topics", page)
        self.assertIn("gca/member-access/", page)
        self.assertNotIn("Platform-Only Evidence Path", page)
        self.assertNotIn("Data Room", page)
        self.assertNotIn('href="data.html"', page)
        ...
        self.assertIn("Remote-control access", page)
        self.assertIn("Base Mainnet", page)
        self.assertIn("GCA/USDT", page)
        self.assertIn("Official channels", page)
        self.assertNotIn("Reviewer Kit", page)
        self.assertNotIn("Platform Replies", page)
        self.assertNotIn("BaseScan Remediation", page)
        self.assertNotIn("Review Queue", page)
        self.assertNotIn("Operations Runbook", page)
        self.assertNotIn("Redacted Review Package Handoff", page)
        self.assertNotIn("Domain email ready for BaseScan resubmission", page)
        self.assertIn(MAINNET_ADDRESS, page)
    ...
    def test_roadmap_page_and_json_define_buildout_boundaries(self):
        ...
        self.assertIn("100 utility credits ledger activation", page)
        self.assertIn("Live for eligible wallet records", page)
        self.assertIn("Device-key service request history", page)
        self.assertIn("public review prompt, follow-up time/count, and redacted settlement state", page)
        ...
        self.assertIn("Member-benefit transfer evidence", page)
        self.assertIn("Live and token-protected; verifies existing exact 10,000 GCA reserve transfers only", page)
        ...
        self.assertIn("Returned 2026-05-23; final package refreshed 2026-08-11; Handoff and Chinese owner flow ready for one support@gcagochina.com submission", page)
        self.assertIn("Latest reviewer package", page)
        self.assertIn("Final package 2026-08-11T18:56:43Z; daily status 2026-08-12T06:37:31Z", page)
        ...
        self.assertIn("official-pool-read-only-snapshot-live", [entry["id"] for entry in roadmap["completedMilestones"]])
        pool_snapshot_milestone = next(entry for entry in roadmap["completedMilestones"] if entry["id"] == "official-pool-read-only-snapshot-live")
        self.assertEqual(pool_snapshot_milestone["completedAt"], "2026-08-11")
        support_priority = next(entry for entry in roadmap["nextBuildPriorities"] if entry["id"] == "support-review-queue")
        self.assertEqual(support_priority["status"], "local-continuity-and-production-member-approval-live")
        self.assertIn("verify_gca_support_review_audit.py", support_priority["verificationCommand"])
        self.assertTrue(support_priority["productionApprovalWorkflowLive"])
        self.assertEqual(support_priority["productionHoldingEvidencePath"], "/gca/holding-verifications")
        ...
        self.assertIn("member-benefit-transfer-evidence-live", [entry["id"] for entry in roadmap["completedMilestones"]])
        self.assertIn(
            "service-request-review-and-credit-settlement-live",
            [entry["id"] for entry in roadmap["completedMilestones"]],
        )
        reviewed_delivery = next(
            entry
            for entry in roadmap["completedMilestones"]
            if entry["id"] == "service-request-review-and-credit-settlement-live"
        )
        self.assertEqual(reviewed_delivery["packetVersion"], "gca_service_request_review_v1")
        ...
        self.assertEqual(transfer_priority["packetVersion"], "gca_member_benefit_transfer_v1")
        self.assertFalse(transfer_priority["automaticTokenTransfer"])
        self.assertIn(
        ...
        self.assertIn(
            "After a manual reserve-wallet transfer, the protected production API verifies and stores successful safe-block evidence for exactly 10,000 GCA from the official reserve to the approved member wallet.",
            roadmap["publicClaimBoundaries"]["safeClaims"],
        )
    ...
    def test_community_page_and_json_define_safe_public_messaging(self):
        ...
        self.assertIn(RADAR_ISSUE_006_PAGE_URL, page)
        ...
        self.assertEqual(community["lastUpdated"], "2026-08-11")
        self.assertEqual(community["currentStage"], "live-product-operations")
        self.assertEqual(community["campaignCycleId"], "product-operations-002")
        ...
        self.assertTrue(any("BaseScan has not published the token profile" in item for item in community["safeAnnouncement"]))
        self.assertTrue(any("ten browser-only risk and research tools" in item for item in community["safeAnnouncement"]))
        ...
        self.assertTrue(any(RADAR_ISSUE_006_PAGE_URL in item for item in community["safeAnnouncement"]))
        self.assertTrue(any(MEMBER_ACCESS_PAGE_URL in item for item in community["safeAnnouncement"]))
        ...
        self.assertEqual(community["campaignCalendar"]["campaignCycleId"], "product-operations-002")
        self.assertEqual(community["campaignCalendar"]["campaignWindow"], "2026-08-11 to 2026-09-07")
        ...
        self.assertEqual(community["publishingDesk"]["campaignCycleId"], "product-operations-002")
        self.assertEqual(community["publishingDesk"]["nextPublishTargetDate"], "2026-08-11")
    ...
    def test_announcements_page_and_json_track_official_content_cadence(self):
        ...
        self.assertIn(MEMBER_ACCESS_PAGE_URL, page)
        self.assertIn(TOOLS_PAGE_URL, page)
        ...
        self.assertEqual(announcements["lastUpdated"], "2026-08-11")
        self.assertEqual(announcements["currentStage"], "live-product-operations")
        self.assertEqual(announcements["campaignCycleId"], "product-operations-002")
        ...
        self.assertEqual(announcements["publicLinks"]["radarIssue006Page"], RADAR_ISSUE_006_PAGE_URL)
        self.assertEqual(announcements["publicLinks"]["radarIssue006"], RADAR_ISSUE_006_URL)
        self.assertEqual(announcements["publicLinks"]["memberAccessPage"], MEMBER_ACCESS_PAGE_URL)
        self.assertEqual(announcements["publicLinks"]["toolsPage"], TOOLS_PAGE_URL)
        self.assertEqual(announcements["publicLinks"]["serviceDeliveryPlaybookPage"], SERVICE_DELIVERY_PLAYBOOK_PAGE_URL)
        self.assertEqual(announcements["publicLinks"]["latestXPost"], LATEST_X_POST_URL)
        self.assertTrue(any(item["topic"] == "Live Product Operations" and item["recommendedLink"] == PRODUCT_PAGE_URL for item in announcements["nextContentQueue"]))
        self.assertTrue(any(item["topic"] == "Member Account Journey" and item["recommendedLink"] == MEMBER_ACCESS_PAGE_URL for item in announcements["nextContentQueue"]))
        self.assertTrue(any(item["topic"] == "Ten Browser Risk Tools" and item["recommendedLink"] == TOOLS_PAGE_URL for item in announcements["nextContentQueue"]))
        ...
        self.assertEqual(announcements["campaignCalendar"]["campaignCycleId"], "product-operations-002")
        self.assertEqual(announcements["campaignCalendar"]["window"], "2026-08-11 to 2026-09-07")
        self.assertEqual(announcements["campaignCalendar"]["nextDraftTargetDate"], "2026-08-11")
        ...
        self.assertEqual(announcements["publishingDesk"]["campaignCycleId"], "product-operations-002")
        self.assertEqual(announcements["publishingDesk"]["nextPublishTargetDate"], "2026-08-11")
    ...
    def test_campaign_page_and_json_publish_30_day_content_calendar(self):
        ...
        self.assertIn("Publishing Desk", page)
        self.assertIn("Every 3 days", page)
        self.assertIn("2026-08-11 to 2026-09-07", page)
        ...
        self.assertIn("Live Product Operations", page)
        self.assertIn("Member Account Journey", page)
        self.assertIn("Ten Browser Risk Tools", page)
        self.assertIn("Product Operations Recap", page)
        self.assertIn("Do Not Publish", page)
        self.assertIn("No return promises", page)
        self.assertIn("Base Mainnet. Fixed supply. Verify the official contract and current feature status first.", page)
        self.assertIn(X_URL, page)
        self.assertIn(TELEGRAM_URL, page)
        self.assertIn(CONTENT_LIBRARY_PAGE_URL, page)
        self.assertIn(PUBLISHING_DESK_PAGE_URL, page)
        ...
        self.assertEqual(campaign["lastUpdated"], "2026-08-11")
        self.assertEqual(campaign["currentStage"], "live-product-operations")
        ...
        self.assertEqual(campaign["nextCopyReadyDraft"]["targetDate"], "2026-08-11")
        self.assertEqual(campaign["nextCopyReadyDraft"]["id"], "gca-draft-2026-08-11-live-product-operations")
        self.assertEqual(campaign["nextCopyReadyDraft"]["topic"], "Live Product Operations")
        ...
        self.assertTrue(any(item["topic"] == "Ten Browser Risk Tools" for item in campaign["contentQueue"]))
        self.assertTrue(any(item["recommendedLink"] == RADAR_ISSUE_006_PAGE_URL for item in campaign["contentQueue"]))
        self.assertTrue(any(item["recommendedLink"] == MEMBER_ACCESS_PAGE_URL for item in campaign["contentQueue"]))
        self.assertTrue(any(item["recommendedLink"] == SERVICE_DELIVERY_PLAYBOOK_PAGE_URL for item in campaign["contentQueue"]))
        ...
        self.assertEqual(campaign["publicLinks"]["radarIssue006Page"], RADAR_ISSUE_006_PAGE_URL)
        self.assertEqual(campaign["publicLinks"]["radarIssue006"], RADAR_ISSUE_006_URL)
        self.assertEqual(campaign["publicLinks"]["memberAccessPage"], MEMBER_ACCESS_PAGE_URL)
        self.assertEqual(campaign["publicLinks"]["toolsPage"], TOOLS_PAGE_URL)
        self.assertEqual(campaign["publicLinks"]["serviceDeliveryPlaybookPage"], SERVICE_DELIVERY_PLAYBOOK_PAGE_URL)
        ...
        self.assertEqual(campaign["publishingDesk"]["nextPublishTargetDate"], "2026-08-11")
    ...
    def test_content_library_page_and_json_publish_bilingual_drafts(self):
        ...
        self.assertIn("Official Market Identity", page)
        self.assertIn("Public Evidence Center", page)
        self.assertIn("Product Operations Recap", page)
        self.assertIn("No return promises", page)
        self.assertIn("Verify the official contract and current feature status first.", page)
        self.assertIn("请先核对官方合约和当前功能状态。", page)
        self.assertIn(X_URL, page)
        self.assertIn(TELEGRAM_URL, page)
        self.assertIn(LATEST_X_POST_URL, page)
        self.assertIn(RADAR_ISSUE_006_PAGE_URL, page)
        ...
        self.assertEqual(library["lastUpdated"], "2026-08-11")
        self.assertEqual(library["currentStage"], "live-product-operations")
        self.assertEqual(library["campaignCycleId"], "product-operations-002")
        ...
        self.assertEqual(library["campaignWindow"]["startDate"], "2026-08-11")
        self.assertEqual(library["campaignWindow"]["endDate"], "2026-09-07")
        ...
        self.assertTrue(any(draft["topic"] == "Live Product Operations" and draft["recommendedLink"] == PRODUCT_PAGE_URL for draft in library["drafts"]))
        self.assertTrue(any(draft["recommendedLink"] == RADAR_ISSUE_006_PAGE_URL for draft in library["drafts"]))
        self.assertTrue(any(draft["recommendedLink"] == MEMBER_ACCESS_PAGE_URL for draft in library["drafts"]))
        self.assertTrue(any(draft["recommendedLink"] == TOOLS_PAGE_URL for draft in library["drafts"]))
        self.assertTrue(any(draft["recommendedLink"] == SERVICE_DELIVERY_PLAYBOOK_PAGE_URL for draft in library["drafts"]))
        ...
        self.assertEqual(library["publicLinks"]["radarIssue006Page"], RADAR_ISSUE_006_PAGE_URL)
        self.assertEqual(library["publicLinks"]["radarIssue006"], RADAR_ISSUE_006_URL)
        self.assertEqual(library["publicLinks"]["memberAccessPage"], MEMBER_ACCESS_PAGE_URL)
        self.assertEqual(library["publicLinks"]["toolsPage"], TOOLS_PAGE_URL)
        self.assertEqual(library["publicLinks"]["serviceDeliveryPlaybookPage"], SERVICE_DELIVERY_PLAYBOOK_PAGE_URL)
    ...
    def test_publishing_desk_page_and_json_define_manual_posting_workflow(self):
        ...
        self.assertIn("Verify the official contract and current feature status first.", page)
        self.assertIn("请先核对官方合约和当前功能状态。", page)
        self.assertIn("Live product-operations update only", page)
        ...
        self.assertEqual(desk["lastUpdated"], "2026-08-11")
        self.assertEqual(desk["currentStage"], "live-product-operations")
        self.assertEqual(desk["campaignCycleId"], "product-operations-002")
        ...
        self.assertEqual(action["targetDate"], "2026-08-11")
        self.assertEqual(action["sourceDraftId"], "gca-draft-2026-08-11-live-product-operations")
        self.assertEqual(action["topic"], "Live Product Operations")
        ...
        self.assertIn("Live product-operations update only", action["claimBoundary"])
        ...
        self.assertEqual(desk["publicLinks"]["radarIssue006"], RADAR_ISSUE_006_PAGE_URL)
        self.assertEqual(desk["publicLinks"]["memberAccess"], MEMBER_ACCESS_PAGE_URL)
        self.assertEqual(desk["publicLinks"]["tools"], TOOLS_PAGE_URL)
    ...
    def test_narrative_page_and_json_define_public_narrative_system(self):
        ...
        self.assertIn("Published production deployment record", page)
        ...
        self.assertIn("Production-live and token-protected", page)
        ...
        self.assertEqual(narrative["lastUpdated"], "2026-08-10")
        ...
        self.assertEqual(live_workflows["account-service-requests"]["status"], "live-manual-review-and-delivery")
        self.assertIn("at most once per request", live_workflows["account-service-requests"]["claimBoundary"])
        ...
        self.assertIn("production service-request and credit-usage Worker routes are live and token-protected", live_workflows["service-delivery-playbook"]["claimBoundary"])
        ...
        self.assertIn("live, token-protected, and verified", live_workflows["worker-routes-handoff"]["claimBoundary"])
        self.assertEqual(live_workflows["official-pool-market-snapshot"]["status"], "live-browser-read-only")
        self.assertEqual(live_workflows["official-pool-market-snapshot"]["verifiedAt"], "2026-08-11")
        self.assertEqual(live_workflows["official-pool-market-snapshot"]["publicUrl"], "https://gcagochina.com/markets.html")
        self.assertIn("not a wallet quote, executable price, trading signal, transaction, or guarantee", live_workflows["official-pool-market-snapshot"]["claimBoundary"])
        self.assertEqual(product_proofs["worker-routes-handoff"]["status"], "production-live-verified")
        ...
        self.assertIn("The controlled account UI, read-only GCA balance verification, eligible ledgers, and manually reviewed service workflow are live.", narrative["publicClaimBoundaries"]["safeClaims"])
    ...
    def test_weekly_radar_issue_005_access_foundation_is_copyable(self):
        ...
        self.assertEqual(radar["status"], "weekly-go-china-radar-issue-005-ready-for-review")
        self.assertEqual(radar["issue"], "issue-005")
        self.assertEqual(radar["issueDate"], "2026-06-14")
        self.assertEqual(radar["publicationStatus"], "operator-review-required-before-social-posting")
        self.assertEqual(radar["chainId"], 8453)
        self.assertEqual(radar["contractAddress"], MAINNET_ADDRESS)
        self.assertIn("not live market data", radar["scope"])
        self.assertIn("not a buy or sell signal", radar["scope"])
        self.assertIn("Public account foundation", [item["name"] for item in radar["narrativeThemes"]])
        self.assertIn("Utility without custody", [item["name"] for item in radar["narrativeThemes"]])
        self.assertIn("Reviewer evidence stays current", [item["name"] for item in radar["narrativeThemes"]])
        self.assertIn("Email Registration", [item["name"] for item in radar["utilityHooks"]])
        self.assertIn("Read-Only Wallet Verification", [item["name"] for item in radar["utilityHooks"]])
        self.assertIn("100 Credits Ledger", [item["name"] for item in radar["utilityHooks"]])
        self.assertIn("GCA Member 30-Day Review", [item["name"] for item in radar["utilityHooks"]])
        self.assertIn("Manual Member Benefit Transfer", [item["name"] for item in radar["utilityHooks"]])
        self.assertIn("No third-party audit has been completed.", radar["riskNotes"])
        self.assertEqual(radar["officialMarket"]["pair"], "GCA/USDT")
        self.assertEqual(radar["officialMarket"]["poolAddress"], OFFICIAL_POOL_ADDRESS)
        self.assertEqual(radar["officialMarket"]["quoteAssetAddress"], BASE_USDT_ADDRESS)
        self.assertEqual(radar["copyReadyPost"]["targetDate"], "2026-06-14")
        self.assertEqual(radar["copyReadyPost"]["channels"], ["X", "Telegram"])
        self.assertTrue(radar["copyReadyPost"]["requiresOperatorReview"])
        self.assertTrue(radar["copyReadyPost"]["xEnglish"])
        self.assertTrue(radar["copyReadyPost"]["xChinese"])
        ...
        self.assertIn("buy or sell signal", radar["publicClaimBoundaries"]["doNotClaim"])
        self.assertIn("return promise", radar["publicClaimBoundaries"]["doNotClaim"])
        self.assertEqual(radar["officialLinks"]["radarIssue005Page"], RADAR_ISSUE_005_PAGE_URL)
        self.assertEqual(radar["officialLinks"]["radarIssue005"], RADAR_ISSUE_005_URL)
        self.assertEqual(radar["officialLinks"]["radarIssue004Page"], RADAR_ISSUE_004_PAGE_URL)
        self.assertEqual(radar["officialLinks"]["radarIssue004"], RADAR_ISSUE_004_URL)
        ...
        self.assertEqual(radar["officialLinks"]["accessPortal"], ACCESS_PAGE_URL)
        self.assertEqual(radar["officialLinks"]["dailyStatus"], DAILY_STATUS_PAGE_URL)
    ...
    def test_service_delivery_playbook_publishes_operator_reviewed_workflow(self):
        ...
        self.assertIn("Service queue, review, follow-up, and credit routes return HTTP 401", page)
        self.assertIn("Copy-Ready Public Summary", page)
        self.assertIn("Service Delivery References", page)
        self.assertIn("Authenticated accounts can submit catalog requests, answer a public more-information prompt, cancel their own queued request before review, and read redacted lifecycle history", page)
        ...
        self.assertEqual(playbook["status"], "service-delivery-playbook-v1-request-followup-live")
        ...
        self.assertTrue(playbook["ledgerBehavior"]["creditUsageLedger"]["linkedToServiceRequest"])
        self.assertTrue(playbook["ledgerBehavior"]["creditUsageLedger"]["atMostOneUsagePerServiceRequest"])
        self.assertTrue(playbook["ledgerBehavior"]["creditUsageLedger"]["serverCatalogUnitAuthoritative"])
        self.assertTrue(playbook["ledgerBehavior"]["creditUsageLedger"]["d1BatchSettlement"])
        ...
        self.assertEqual(
            playbook["routeStatus"]["serviceRequestReviewWorkerEndpoint"],
        ...
        self.assertEqual(playbook["routeStatus"]["d1VisibilityStatus"], "passed")
        ...
        self.assertEqual(playbook["routeStatus"]["latestReadinessCheckAt"], "2026-08-10T13:21:00Z")
        self.assertEqual(playbook["routeStatus"]["latestPublicRouteCheckAt"], "2026-08-10T13:23:22Z")
        ...
        self.assertEqual(playbook["routeStatus"]["workerVersionId"], "def4a0ea-fcbb-4d0e-a380-ba9656d7dc05")
        ...
        self.assertIn(
            "Production service-request, service-review, and credit-usage Worker routes are live and token-protected after all release gates passed.",
            playbook["releaseBoundaries"],
        )
        ...
        self.assertIn("service delivery is automatic or does not require manual approval", playbook["publicClaimBoundaries"]["doNotClaim"])
        ...
        self.assertEqual(playbook["officialLinks"]["apiStatusPage"], API_STATUS_PAGE_URL)
        self.assertEqual(playbook["officialLinks"]["apiStatus"], API_STATUS_URL)
    ...
    def test_worker_routes_handoff_publishes_production_route_deploy_record(self):
        page = (ROOT / "site" / "worker-routes-handoff.html").read_text()
        handoff = json.loads((ROOT / "site" / "worker-routes-handoff.json").read_text())

        self.assertIn("GCA Worker Routes Deployment", page)
        self.assertIn("Cloudflare Worker Deployment Record", page)
        self.assertIn("Production-live and protected", page)
        self.assertIn("All required gates passed", page)
        self.assertIn("HTTP 401", page)
        self.assertIn("docs/gca_worker_pending_routes_deploy_handoff.md", page)
        self.assertIn("Already Live Routes", page)
        self.assertIn("Newly Released Routes", page)
        ...
        self.assertIn("python3 tools/check_gca_registration_api.py --public-only --timeout 30 --include-service-routes", page)
        self.assertIn("python3 tools/check_gca_registration_api.py --token-file cloudflare/gca-registration-worker/.env.admin.local --limit 5 --include-service-routes", page)
        ...
        self.assertIn("0015_service_request_followups.sql", page)
        ...
        self.assertEqual(handoff["status"], "worker-routes-v1-request-followup-production-live")
        ...
        self.assertEqual(handoff["currentStatus"]["latestReadinessCheckAt"], "2026-08-10T13:21:00Z")
        self.assertEqual(handoff["currentStatus"]["latestPublicRouteCheckAt"], "2026-08-10T13:23:22Z")
        self.assertEqual(handoff["currentStatus"]["latestAdminRouteCheckAt"], "2026-08-10T13:23:40Z")
        ...
        self.assertEqual(handoff["currentStatus"]["workerDeployPermission"], "passed")
        self.assertFalse(handoff["currentStatus"]["code10000Seen"])
        self.assertEqual(handoff["currentStatus"]["pendingRouteAnonymousGetStatus"], {
            "/gca/service-requests": 401,
            "/gca/credit-usage": 401,
            "/gca/service-request-reviews": 401,
            "/gca/service-request-followups": 401,
        ...
        self.assertEqual(prepared_routes["/gca/service-requests"]["status"], "live-token-protected")
        self.assertTrue(prepared_routes["/gca/service-requests"]["productionLive"])
        self.assertFalse(prepared_routes["/gca/service-requests"]["createsTradingPermission"])
        self.assertEqual(prepared_routes["/gca/service-request-reviews"]["status"], "live-token-protected")
        self.assertEqual(
            prepared_routes["/gca/service-request-reviews"]["packetVersion"],
            "gca_service_request_review_v1",
        )
        self.assertTrue(prepared_routes["/gca/service-request-reviews"]["appendOnlyReviews"])
        self.assertTrue(prepared_routes["/gca/service-request-reviews"]["approvedBeforeDeliveryRequired"])
        self.assertTrue(prepared_routes["/gca/service-request-reviews"]["atMostOneCreditUsagePerRequest"])
        self.assertEqual(prepared_routes["/gca/credit-usage"]["status"], "live-token-protected")
        self.assertTrue(prepared_routes["/gca/credit-usage"]["writtenOnlyAfterReviewedDelivery"])
        self.assertTrue(prepared_routes["/gca/credit-usage"]["productionLive"])
        ...
        self.assertIn("0015_service_request_followups.sql", gates["remote-d1-migrations"]["expectedMigrations"])
        self.assertIn("--include-service-routes", gates["post-deploy-public-smoke"]["command"])
        self.assertIn("--include-service-routes", gates["post-deploy-admin-smoke"]["command"])
        ...
        self.assertIn(
            "The GCA service-request, manual review, and credit settlement Worker routes are production-live and token-protected.",
            handoff["safeClaims"],
        )
        self.assertIn("service delivery is automatic or bypasses manual approval", handoff["doNotClaim"])
    ...
    def test_member_ledger_page_and_json_define_public_schema(self):
        ...
        self.assertEqual(ledger["status"], "public-member-ledger-and-device-status-live")
        ...
        self.assertEqual(ledger["preparedPaths"]["memberBenefitTransfers"], "/gca/member-benefit-transfers")
        ...
        self.assertIn("memberBenefitTransferRecord", ledger["recordSchemas"])
        ...
        self.assertIn("memberBenefitTransferRecordId", member_fields)
        self.assertIn("memberBenefitTransferVerifiedAt", member_fields)
        self.assertIn("memberBenefitTransferVerificationStatus", member_fields)
        transfer_record = ledger["recordSchemas"]["memberBenefitTransferRecord"]
        self.assertEqual(transfer_record["packetVersion"], "gca_member_benefit_transfer_v1")
        self.assertEqual(transfer_record["endpoint"], "/gca/member-benefit-transfers")
        self.assertEqual(transfer_record["officialSourceWallet"], "0x5e8F84748612B913aAcC937492AC25dc5630E246")
        self.assertEqual(transfer_record["exactTransferAmount"], "10000 GCA")
        self.assertTrue(transfer_record["requiresSuccessfulReceiptAtOrBelowSafeBlock"])
        self.assertTrue(transfer_record["requiresApprovedMemberWalletRecipient"])
        self.assertFalse(transfer_record["automaticTokenTransfer"])
        self.assertFalse(transfer_record["requiresSignature"])
        self.assertFalse(transfer_record["writesWallet"])
    ...
    def test_member_benefit_transfer_page_and_json_define_manual_runbook(self):
        ...
        self.assertIn("/gca/member-benefit-transfers", page)
        self.assertIn("gca_member_benefit_transfer_v1", page)
        ...
        self.assertEqual(transfer["status"], "public-member-benefit-transfer-evidence-workflow-live")
        ...
        self.assertTrue(transfer["transferPolicy"]["productionEvidenceVerificationLive"])
        self.assertEqual(transfer["transferPolicy"]["productionEvidencePacketVersion"], "gca_member_benefit_transfer_v1")
        self.assertTrue(transfer["transferPolicy"]["recipientMustEqualVerifiedMemberWallet"])
        self.assertIn("memberBenefitClaimStatus is pending_manual_reserve_transfer", transfer["approvalPrerequisites"])
        self.assertIn("no prior member-benefit transfer evidence exists for the same member ledger", transfer["approvalPrerequisites"])
        ...
        self.assertIn("transactionHash", transfer["requiredLedgerFieldsAfterTransfer"])
        self.assertIn("safeSnapshotBlockNumber", transfer["requiredLedgerFieldsAfterTransfer"])
        self.assertIn("verificationStatus", transfer["requiredLedgerFieldsAfterTransfer"])
        ...
        workflow = transfer["productionEvidenceWorkflow"]
        self.assertEqual(workflow["packetVersion"], "gca_member_benefit_transfer_v1")
        self.assertEqual(workflow["endpoint"], "/gca/member-benefit-transfers")
        self.assertEqual(workflow["officialSourceWallet"], RESERVE_WALLET)
        self.assertEqual(workflow["operatorTool"], "tools/record_cloudflare_member_benefit_transfer.py")
        self.assertTrue(workflow["requiresReceiptAtOrBelowSafeBlock"])
        self.assertFalse(workflow["automaticTokenTransfer"])
        self.assertEqual(workflow["recordsObservedDuringDeploymentSmoke"], 0)
        ...
        self.assertIn("production Worker verifies", " ".join(transfer["publicClaimBoundaries"]["safeClaims"]))
    ...
    def test_zh_member_benefit_transfer_page_explains_manual_runbook(self):
        ...
        self.assertIn("中文会员权益转账流程 / 2026-07-27", page)
        ...
        self.assertIn("不能说：证据接口会发起、签名、授权或广播代币转账", page)
    ...
    def test_utility_page_connects_gca_to_quant_tools_safely(self):
        ...
        self.assertEqual(
            utility_json["positioning"]["publicStatus"],
        ...
        self.assertIn("finished connected market-data or trading utility before those modules are released", utility_json["publicClaimBoundaries"]["doNotClaim"])
    ...
    def test_product_spec_page_and_json_define_product_modules_safely(self):
        ...
        self.assertIn("GCA AI Quant Access", page)
        self.assertIn("Research clearly. Control risk.", page)
        self.assertIn("10 browser-based tools", page)
        self.assertIn("One workspace for the full risk cycle.", page)
        self.assertNotIn("Platform-Only Evidence Path", page)
        self.assertNotIn("Data Room", page)
        self.assertNotIn('href="data.html"', page)
        for forbidden in (
        ...
        self.assertIn("Research Notes", page)
        ...
        self.assertIn("Member Workspace", page)
        self.assertIn('href="member-workspace.html"', page)
        self.assertIn("Security boundaries", page)
        self.assertIn("No custody", page)
        self.assertIn("withdrawal permission", page)
        self.assertIn("exchange API secrets", page)
        self.assertIn("100 credits", page)
        self.assertIn("1,000,000 GCA", page)
        self.assertIn("Official on-chain identity", page)
        self.assertIn('href="zh-product.html"', page)
        self.assertIn(MAINNET_ADDRESS, page)
        self.assertIn(BASE_USDT_ADDRESS, page)
        self.assertIn(OFFICIAL_POOL_ADDRESS, page)
        self.assertNotIn("Product Spec", page)
        self.assertNotIn("Current Stage", page)
        self.assertNotIn("Public Account UI", page)
        self.assertNotIn(OLD_WETH_POOL_ADDRESS, page)
        self.assertNotIn("GCA/WETH", page)
        ...
        research_notes = next(item for item in product["productModules"] if item["id"] == "member-research-notes")
        self.assertEqual(research_notes["status"], "public-client-side-preview-live")
        self.assertEqual(research_notes["publicUrl"], "https://gcagochina.com/research-notes.html")
        self.assertTrue(research_notes["browserLocalNotes"])
        self.assertTrue(research_notes["portableJsonBackup"])
        ...
        self.assertTrue(portfolio_risk["browserLocalPortfolio"])
        self.assertTrue(portfolio_risk["portableJsonBackup"])
    ...
    def test_public_product_stage_and_basescan_description_stay_aligned(self):
        product = json.loads((ROOT / "site" / "product.json").read_text())
        roadmap = json.loads((ROOT / "site" / "roadmap.json").read_text())
        project = json.loads((ROOT / "site" / "project.json").read_text())
        platform_values = json.loads((ROOT / "launch" / "data_platform_form_values.json").read_text())
        basescan_values = json.loads((ROOT / "launch" / "basescan_resubmission_values.json").read_text())
        final_package = json.loads((ROOT / "launch" / "basescan_final_submission_package.json").read_text())
        handoff = json.loads((ROOT / "site" / "basescan-handoff.json").read_text())
        canonical_pages = "\n".join(
        ...
        stages = {
            ...
            project["productSpec"]["currentStage"],
            project["roadmap"]["currentStage"],
            platform_values["productSpecPositioning"]["currentStage"],
            platform_values["utilityPositioning"]["roadmap"]["currentStage"],
        }
        self.assertEqual(stages, {PRODUCT_STAGE})
        ...
        self.assertIn("ten browser-only public risk and research tools", canonical_pages.lower())
        ...
        self.assertEqual(final_package["formFields"]["basicInformation"]["projectDescription"], description)
        self.assertIn(description, handoff["finalSubmissionPackage"]["copyPasteContent"]["basicInformationPlainText"])
        self.assertIn(description, canonical_pages)
        self.assertIn("do not fetch live market data, connect exchanges, or place orders", description)
        self.assertNotIn("planned access to non-custodial quant risk tools", description)
    ...
    def test_access_portal_page_and_json_define_live_account_path(self):
        ...
        for forbidden in (
            ...
            self.assertNotIn(forbidden, page)
        self.assertIn("account UI + redacted status live", page)
        ...
        self.assertEqual(access["currentState"]["memberAccessPacketVersion"], "gca_member_access_v2")
        self.assertEqual(access["currentState"]["accountStatusPacketVersion"], "gca_account_status_v1")
        self.assertTrue(access["currentState"]["serverAccountStatusLive"])
        self.assertTrue(access["currentState"]["serverAccountStatusReadOnly"])
        self.assertTrue(access["currentState"]["statusAccessTokenStoredAsSha256"])
        self.assertFalse(access["currentState"]["serverAccountStatusReturnsEmail"])
        self.assertFalse(access["currentState"]["serverAccountStatusReturnsFullWallet"])
        self.assertFalse(access["currentState"]["serverAccountStatusReturnsDeviceKey"])
        ...
        self.assertTrue(access["securityBoundaries"]["readOnlyAccountStatus"])
        self.assertTrue(access["securityBoundaries"]["accountStatusTokenStoredAsSha256"])
        self.assertFalse(access["securityBoundaries"]["accountStatusReturnsEmail"])
        self.assertFalse(access["securityBoundaries"]["accountStatusReturnsFullWallet"])
        self.assertFalse(access["securityBoundaries"]["accountStatusReturnsAccessToken"])
    ...
    def test_access_api_page_and_json_define_live_member_access_routes(self):
        ...
        for forbidden in (
            ...
            self.assertNotIn(forbidden, page)
        self.assertIn("public follow-up and reviewed settlement live", page)
        self.assertIn("Account + redacted review history live", page)
        self.assertIn("device-key protected service requests and follow-ups, queued-request cancellation, request history, safe key rotation, wallet verification", page)
        self.assertIn("tools/gca_member_backend.py", page)
        self.assertIn("tools/export_gca_review_package.py", page)
        ...
        self.assertIn("/gca/member-benefit-transfers", page)
        self.assertIn("gca_member_benefit_transfer_v1", page)
        self.assertIn("exactly 10,000 GCA", page)
        self.assertIn("eth_call", page)
        self.assertIn("balanceOf", page)
        self.assertIn("100 GCA AI Quant Access credits", page)
        self.assertIn("GCA Member", page)
        ...
        self.assertIn("15-minute same-rotation retry", page)
        ...
        self.assertEqual(api["status"], "public-access-api-request-followup-live")
        ...
        self.assertEqual(api["currentState"]["currentStage"], "account-service-request-followup-api-live")
        ...
        self.assertEqual(api["currentState"]["memberAccessPacketVersion"], "gca_member_access_v2")
        self.assertEqual(api["currentState"]["legacyMemberAccessPacketVersion"], "gca_member_access_v1")
        self.assertEqual(api["currentState"]["accountStatusPacketVersion"], "gca_account_status_v1")
        self.assertTrue(api["currentState"]["accountStatusProductionLive"])
        self.assertTrue(api["currentState"]["accountStatusReadOnly"])
        self.assertFalse(api["currentState"]["accountStatusReturnsEmail"])
        self.assertFalse(api["currentState"]["accountStatusReturnsAccessToken"])
        self.assertEqual(api["currentState"]["accountStatusRotationPacketVersion"], "gca_account_status_rotation_v1")
        self.assertTrue(api["currentState"]["accountStatusRotationProductionLive"])
        self.assertEqual(api["currentState"]["accountStatusRotationGraceMinutes"], 15)
        self.assertFalse(api["currentState"]["accountStatusRotationReturnsAccessToken"])
        self.assertFalse(api["currentState"]["accountStatusRotationChangesAccountOrLedgers"])
        ...
        self.assertTrue(api["currentState"]["creditUsageLedgerWritesLive"])
        self.assertFalse(api["currentState"]["creditUsageWorkerDeployBlocked"])
        ...
        self.assertTrue(api["currentState"]["serviceRequestReviewProductionLive"])
        self.assertEqual(
            api["currentState"]["serviceRequestReviewPacketVersion"],
            "gca_service_request_review_v1",
        )
        self.assertTrue(api["currentState"]["serviceRequestReviewAppendOnly"])
        self.assertTrue(api["currentState"]["serviceRequestReviewApprovedBeforeDeliveryRequired"])
        self.assertTrue(api["currentState"]["serviceRequestReviewServerCatalogAuthoritative"])
        ...
        self.assertTrue(api["currentState"]["holdingHistoryVerificationProductionLive"])
        self.assertTrue(api["currentState"]["onchainHoldingHistoryRequiredForApproval"])
        self.assertEqual(api["currentState"]["holdingVerificationPacketVersion"], "gca_holding_verification_v1")
        self.assertEqual(api["currentState"]["memberBenefitTransferPacketVersion"], "gca_member_benefit_transfer_v1")
        self.assertTrue(api["currentState"]["memberBenefitTransferEvidenceProductionLive"])
        self.assertTrue(api["currentState"]["memberBenefitTransferStillManual"])
        ...
        self.assertEqual(api["productionEmailRegistrationBackend"]["accountStatusRotationMigration"], "cloudflare/gca-registration-worker/migrations/0010_account_status_rotation.sql")
        self.assertEqual(api["productionEmailRegistrationBackend"]["memberReviewOperatorTool"], "tools/review_cloudflare_member.py")
        self.assertEqual(api["productionEmailRegistrationBackend"]["memberBenefitTransferOperatorTool"], "tools/record_cloudflare_member_benefit_transfer.py")
        ...
        self.assertEqual(api["productionEmailRegistrationBackend"]["accountStatusEndpoint"], "https://gca-registration-api.gcagochina.workers.dev/gca/account-status")
        self.assertEqual(api["productionEmailRegistrationBackend"]["accountStatusRotationEndpoint"], "https://gca-registration-api.gcagochina.workers.dev/gca/account-status/rotate")
        self.assertEqual(api["productionEmailRegistrationBackend"]["accountStatusRotationVersion"], "gca_account_status_rotation_v1")
        self.assertEqual(api["productionEmailRegistrationBackend"]["accountStatusRotationGraceMinutes"], 15)
        ...
        self.assertEqual(api["productionEmailRegistrationBackend"]["adminMemberBenefitTransfersEndpoint"], "https://gca-registration-api.gcagochina.workers.dev/gca/member-benefit-transfers")
        self.assertEqual(api["productionEmailRegistrationBackend"]["memberBenefitTransferVersion"], "gca_member_benefit_transfer_v1")
        self.assertEqual(api["productionEmailRegistrationBackend"]["memberBenefitTransferOfficialSourceWallet"], RESERVE_WALLET)
        ...
        self.assertEqual(api["productionEmailRegistrationBackend"]["pendingRoutePostDeployPublicSmokeCommand"], "python3 tools/check_gca_registration_api.py --public-only --timeout 30 --include-service-routes")
        self.assertEqual(api["productionEmailRegistrationBackend"]["pendingRoutePostDeployAdminSmokeCommand"], "python3 tools/check_gca_registration_api.py --token-file cloudflare/gca-registration-worker/.env.admin.local --limit 5 --include-service-routes")
        ...
        self.assertIn("admin smoke check passes with --include-service-routes", api["productionEmailRegistrationBackend"]["pendingRouteReleaseGates"])
        ...
        self.assertTrue(api["securityModel"]["accountStatusReadOnly"])
        self.assertTrue(api["securityModel"]["accountStatusUsesDeviceKey"])
        ...
        self.assertTrue(api["securityModel"]["memberBenefitTransferEvidenceUsesD1BatchTransaction"])
        self.assertTrue(api["securityModel"]["memberBenefitTransferEvidenceRequiresSafeBlock"])
        self.assertTrue(api["securityModel"]["memberBenefitTransferEvidenceRequiresOfficialReserveSender"])
        self.assertEqual(api["securityModel"]["memberBenefitTransferEvidenceRequiresExactAmount"], "10000 GCA")
        self.assertFalse(api["securityModel"]["memberBenefitTransferSendsTokens"])
        ...
        for endpoint in api["endpoints"]:
            if endpoint["id"] in {"operator-summary", "operator-digest", "operator-action-plan", "review-package", "member-review-update"}:
            ...
            elif endpoint["id"] in {"email-registrations-create", "contact-suppressions-create", "access-config-read", "member-access-create", "account-status-read", "account-status-rotation", "account-status-recovery-request", "account-status-recovery-completion", "account-service-requests-create", "account-service-requests-status", "account-service-request-followups-create", "account-service-delivery-receipts-create", "account-service-request-cancellations-create", "wallet-verifications"}:
                ...
                "service-request-reviews-read-write",
                "service-request-followups-read",
                "credit-usage-read",
                "credit-usage-create",
            }:
        ...
        self.assertEqual(api["memberAccessPacketVersion"], "gca_member_access_v2")
        ...
        self.assertIn("statusAccessToken", member_access["requiredRequestFields"])
        self.assertTrue(any("SHA-256" in item for item in member_access["serverChecks"]))
        ...
        account_status = next(item for item in api["endpoints"] if item["id"] == "account-status-read")
        self.assertEqual(account_status["path"], "/gca/account-status")
        self.assertIn("statusAccessToken", account_status["requiredRequestFields"])
        self.assertTrue(any("D1 reads only" in item for item in account_status["serverChecks"]))
        self.assertTrue(any("omits email" in item for item in account_status["serverChecks"]))
        rotation = next(item for item in api["endpoints"] if item["id"] == "account-status-rotation")
        self.assertEqual(rotation["path"], "/gca/account-status/rotate")
        self.assertIn("currentStatusAccessToken", rotation["requiredRequestFields"])
        self.assertIn("newStatusAccessToken", rotation["requiredRequestFields"])
        self.assertTrue(any("old key cannot read account status" in item for item in rotation["serverChecks"]))
        self.assertTrue(any("15 minutes" in item for item in rotation["serverChecks"]))
        self.assertTrue(any("not changed" in item for item in rotation["serverChecks"]))
        self.assertNotIn("statusAccessToken", rotation["responseFields"])
        ...
        service_request_review_endpoint = next(
            item for item in api["endpoints"] if item["id"] == "service-request-reviews-read-write"
        )
        self.assertIn("clientReviewId", service_request_review_endpoint["requiredRequestFields"])
        self.assertIn("acknowledgements.deliveryCompleted", service_request_review_endpoint["deliveryRequestFields"])
        self.assertIn(
            "delivered is allowed only after approved_operator_review",
            service_request_review_endpoint["serverChecks"],
        )
        self.assertIn(
            "one serviceRequestId can create at most one credit usage record",
            service_request_review_endpoint["serverChecks"],
        )
        self.assertIn("delivered", service_request_review_endpoint["allowedDecisions"])
        ...
        self.assertIn("authorization bearer token", transfer_endpoint["requiredRequestFields"])
        self.assertIn("packetVersion", transfer_endpoint["requiredRequestFields"])
        self.assertIn("transactionHash", transfer_endpoint["requiredRequestFields"])
        self.assertIn("acknowledgements.manualReserveTransferCompleted", transfer_endpoint["requiredRequestFields"])
        self.assertTrue(any("safe-block reads are read-only" in item for item in transfer_endpoint["serverChecks"]))
        self.assertIn("receipt sender must be the published official reserve wallet", transfer_endpoint["serverChecks"])
        self.assertTrue(any("exactly 10000 GCA" in item for item in transfer_endpoint["serverChecks"]))
        ...
        self.assertIn("authorization bearer token", transfer_read_endpoint["requiredRequestFields"])
        self.assertIn("verificationStatus", transfer_read_endpoint["responseFields"])
        self.assertIn("safeSnapshotBlockNumber", transfer_read_endpoint["responseFields"])
        self.assertIn("transactionHash", transfer_read_endpoint["optionalRequestFields"])
        ...
        self.assertIn("GCA has a live public access API for email registration, contact suppression, access config, member access, device-key account status, and read-only wallet verification.", api["publicClaimBoundaries"]["safeClaims"])
        self.assertIn("GCA operators can record reviewed service-level credit usage against an existing credit ledger through the live token-protected Worker route; anonymous reads are rejected.", api["publicClaimBoundaries"]["safeClaims"])
    ...
    def test_cloudflare_registration_backend_is_deployable_and_safe(self):
        ...
        self.assertIn("The current Worker account does not contain that zone", docs)
    ...
    def test_access_operations_page_and_json_define_manual_operations_runbook(self):
        ...
        for forbidden in (
            ...
            self.assertNotIn(forbidden, page)
        self.assertIn("account follow-up and manual settlement live", page)
        self.assertIn("account intake", page)
        ...
        self.assertEqual(ops["status"], "public-access-operations-request-followup-live")
        ...
        self.assertEqual(ops["currentState"]["currentStage"], "account-request-followup-and-manual-settlement-live")
        ...
        self.assertTrue(ops["currentState"]["creditUsageLedgerWritesLive"])
        self.assertFalse(ops["currentState"]["creditUsageWorkerDeployBlocked"])
        ...
        self.assertEqual(ops["currentState"]["serviceRequestReviewAdminPath"], "/gca/service-request-reviews")
        self.assertEqual(
            ops["currentState"]["serviceRequestReviewOperatorTool"],
            "tools/review_cloudflare_service_request.py",
        )
        self.assertTrue(ops["currentState"]["serviceRequestDeliveryRequiresApprovedReview"])
        self.assertTrue(ops["currentState"]["serviceRequestCreditSettlementAtMostOnce"])
        ...
        self.assertFalse(ops["currentState"]["localSupportReviewContinuityImmutable"])
        self.assertFalse(ops["currentState"]["productionSupportApprovalWorkflowLive"])
        ...
        self.assertEqual(member_ops["serviceRequestReviewVersion"], "gca_service_request_review_v1")
        self.assertEqual(
            member_ops["serviceRequestReviewOperatorTool"],
            "tools/review_cloudflare_service_request.py",
        )
        self.assertEqual(
        ...
        self.assertIn("serviceRequestReviewId", ops["requiredReviewRecord"])
        self.assertIn("serviceRequestReviewDecision", ops["requiredReviewRecord"])
        self.assertIn("serviceRequestReviewReasonCode", ops["requiredReviewRecord"])
        self.assertIn("serviceRequestDeliveryReference", ops["requiredReviewRecord"])
        ...
        self.assertIn("auditRecordHash", ops["requiredReviewRecord"])
        self.assertIn("auditLegacyPrefixSha256", ops["requiredReviewRecord"])
        ...
        self.assertTrue(ops["operatorControls"]["memberBenefitTransferEvidenceProductionLive"])
        self.assertEqual(ops["operatorControls"]["memberBenefitTransferPacketVersionMustEqual"], "gca_member_benefit_transfer_v1")
        self.assertEqual(ops["operatorControls"]["memberBenefitTransferSourceWalletMustEqual"], "0x5e8F84748612B913aAcC937492AC25dc5630E246")
        self.assertEqual(ops["operatorControls"]["memberBenefitTransferAmountMustEqual"], "10000 GCA")
        self.assertTrue(ops["operatorControls"]["memberBenefitTransferEvidenceRequiresSafeBlock"])
        self.assertTrue(ops["operatorControls"]["memberBenefitTransferEvidenceRequiresSuccessfulReceipt"])
        self.assertTrue(ops["operatorControls"]["memberBenefitTransferEvidenceDoesNotSendTokens"])
        ...
        self.assertEqual(
            ops["operatorControls"]["serviceRequestReviewPacketVersionMustEqual"],
            "gca_service_request_review_v1",
        )
        self.assertTrue(ops["operatorControls"]["serviceRequestReviewAdminTokenRequired"])
        self.assertTrue(ops["operatorControls"]["serviceRequestReviewAppendOnly"])
        self.assertTrue(ops["operatorControls"]["serviceRequestDeliveryRequiresApprovedReview"])
        self.assertTrue(ops["operatorControls"]["serviceRequestDeliveryUsesServerCatalogCreditUnit"])
        self.assertTrue(ops["operatorControls"]["serviceRequestDeliveryDeductsCreditsAtMostOnce"])
        self.assertTrue(ops["operatorControls"]["serviceRequestDeliveryUsesD1BatchTransaction"])
        self.assertFalse(ops["operatorControls"]["serviceRequestHistoryReturnsOperatorNote"])
        self.assertFalse(ops["operatorControls"]["serviceRequestHistoryReturnsReviewerIdentity"])
        ...
        self.assertIn("verify the complete local support-review chain with tools/verify_gca_support_review_audit.py", package_handoff["requiredBeforeSharing"])
        ...
        self.assertEqual(ops["memberAccessOpsPipeline"]["pendingRoutePostDeployPublicSmokeCommand"], "python3 tools/check_gca_registration_api.py --public-only --timeout 30 --include-service-routes")
        self.assertEqual(ops["memberAccessOpsPipeline"]["pendingRoutePostDeployAdminSmokeCommand"], "python3 tools/check_gca_registration_api.py --token-file cloudflare/gca-registration-worker/.env.admin.local --limit 5 --include-service-routes")
        ...
        self.assertEqual(ops["memberAccessOpsPipeline"]["memberBenefitTransferEndpoint"], "https://gca-registration-api.gcagochina.workers.dev/gca/member-benefit-transfers")
        self.assertEqual(ops["memberAccessOpsPipeline"]["memberBenefitTransferOperatorTool"], "tools/record_cloudflare_member_benefit_transfer.py")
        self.assertEqual(ops["memberAccessOpsPipeline"]["memberBenefitTransferMigration"], "cloudflare/gca-registration-worker/migrations/0008_member_benefit_transfer_evidence.sql")
        self.assertTrue(ops["memberAccessOpsPipeline"]["boundaries"]["memberBenefitEvidenceWritesProductionData"])
        self.assertTrue(ops["memberAccessOpsPipeline"]["boundaries"]["memberBenefitEvidenceVerifiesExistingTransactionOnly"])
        self.assertTrue(ops["memberAccessOpsPipeline"]["boundaries"]["memberBenefitEvidenceDoesNotSendTokens"])
        ...
        self.assertIn("public smoke check passes with --include-service-routes", ops["memberAccessOpsPipeline"]["pendingRouteReleaseGates"])
        ...
        self.assertIn("Submitted holding dates and transaction hashes are supporting preview evidence only and cannot activate GCA Member automatically.", ops["publicClaimBoundaries"]["safeClaims"])
    ...
    def test_credits_catalog_page_and_json_define_reviewed_service_units(self):
        ...
        self.assertIn("approval required before delivery", page)
        self.assertIn("settled at most once after completed delivery", page)
        self.assertIn("100 GCA AI Quant Access credits", page)
        self.assertIn("10,000 GCA", page)
        self.assertIn("1,000,000 GCA", page)
        self.assertIn("10,000 GCA member benefit", page)
        ...
        self.assertEqual(credits["status"], "public-credits-catalog-request-followup-live")
        ...
        self.assertEqual(
            ...
            "review-prompt-followup-and-reviewed-settlement-live",
        )
        ...
        self.assertTrue(credits["currentState"]["creditUsageLedgerWritesLive"])
        self.assertFalse(credits["currentState"]["creditUsageWorkerDeployBlocked"])
        ...
        for item in credits["serviceCatalog"]:
            if item["id"] == "member-research-notes":
                self.assertEqual(item["status"], "live-member-manual-review-service-unit")
                self.assertEqual(item["unitType"], "reviewed member service credit unit")
            elif item["id"] == "support-review-queue":
                self.assertEqual(item["status"], "live-member-priority-review-workflow")
                self.assertEqual(item["unitType"], "member workflow priority")
            else:
                self.assertEqual(item["status"], "live-manual-review-service-unit")
                self.assertEqual(item["unitType"], "reviewed service credit unit")
        research_notes = next(item for item in credits["serviceCatalog"] if item["id"] == "member-research-notes")
        self.assertEqual(research_notes["publicPreview"]["status"], "live-client-side-preview")
        self.assertEqual(research_notes["publicPreview"]["url"], "https://gcagochina.com/research-notes.html")
        self.assertTrue(research_notes["publicPreview"]["browserLocalNotes"])
        self.assertTrue(research_notes["publicPreview"]["portableJsonBackup"])
        self.assertFalse(research_notes["publicPreview"]["storesOnServer"])
        self.assertFalse(research_notes["publicPreview"]["deductsCredits"])
        portfolio_risk = next(item for item in credits["serviceCatalog"] if item["id"] == "portfolio-risk-map")
        self.assertEqual(portfolio_risk["creditUnit"], 15)
        self.assertEqual(portfolio_risk["publicPreview"]["status"], "live-client-side-preview")
        self.assertEqual(portfolio_risk["publicPreview"]["url"], "https://gcagochina.com/portfolio-risk.html")
        self.assertTrue(portfolio_risk["publicPreview"]["browserLocalPortfolio"])
        self.assertTrue(portfolio_risk["publicPreview"]["portableJsonBackup"])
        for field in ("storesOnServer", "deductsCredits", "connectsWallet", "connectsExchange", "fetchesMarketData", "placesOrders"):
            self.assertFalse(portfolio_risk["publicPreview"][field])
        self.assertEqual(credits["usageLedger"]["status"], "live-token-protected-operator-credit-usage-ledger")
        self.assertTrue(credits["usageLedger"]["productionWorkerEndpointLive"])
        ...
        self.assertEqual(credits["usageLedger"]["postDeployPublicSmokeCommand"], "python3 tools/check_gca_registration_api.py --public-only --timeout 30 --include-service-routes")
        self.assertEqual(credits["usageLedger"]["postDeployAdminSmokeCommand"], "python3 tools/check_gca_registration_api.py --token-file cloudflare/gca-registration-worker/.env.admin.local --limit 5 --include-service-routes")
        self.assertIn("cloudflare-auth-session passes", credits["usageLedger"]["releaseGatesBeforeLive"])
        self.assertIn("admin smoke check passes with --include-service-routes", credits["usageLedger"]["releaseGatesBeforeLive"])
        ...
        self.assertEqual(credits["serviceRequestQueue"]["status"], "live-token-protected-operator-service-request-queue")
        self.assertTrue(credits["serviceRequestQueue"]["productionWorkerEndpointLive"])
        ...
        self.assertEqual(credits["serviceRequestQueue"]["postDeployPublicSmokeCommand"], "python3 tools/check_gca_registration_api.py --public-only --timeout 30 --include-service-routes")
        self.assertEqual(credits["serviceRequestQueue"]["postDeployAdminSmokeCommand"], "python3 tools/check_gca_registration_api.py --token-file cloudflare/gca-registration-worker/.env.admin.local --limit 5 --include-service-routes")
        self.assertIn("D1 visibility passes", credits["serviceRequestQueue"]["releaseGatesBeforeLive"])
        self.assertIn("public smoke check passes with --include-service-routes", credits["serviceRequestQueue"]["releaseGatesBeforeLive"])
        ...
        service_review = credits["serviceRequestReview"]
        self.assertEqual(service_review["status"], "live-token-protected-append-only-reviewed-delivery")
        self.assertEqual(
            service_review["endpoint"],
            "https://gca-registration-api.gcagochina.workers.dev/gca/service-request-reviews",
        )
        self.assertEqual(service_review["packetVersion"], "gca_service_request_review_v1")
        self.assertEqual(service_review["operatorTool"], "tools/review_cloudflare_service_request.py")
        self.assertTrue(service_review["approvedBeforeDeliveryRequired"])
        self.assertTrue(service_review["serverCatalogCreditUnitAuthoritative"])
        self.assertTrue(service_review["creditsDeductedOnlyOnDelivered"])
        self.assertTrue(service_review["creditsDeductedAtMostOncePerServiceRequest"])
        self.assertTrue(service_review["usesD1BatchTransaction"])
        self.assertFalse(service_review["automaticWalletAction"])
        self.assertFalse(service_review["automaticTokenTransfer"])
        self.assertFalse(service_review["createsTradingPermission"])
        ...
        self.assertIn("Operator-reviewed service delivery can be recorded through the live token-protected GCA credit usage Worker route with before/after remaining credits; anonymous reads are rejected.", credits["publicClaimBoundaries"]["safeClaims"])
    ...
    def test_release_gates_page_and_json_define_go_live_boundaries(self):
        ...
        for forbidden in (
            ...
            self.assertNotIn(forbidden, page)
        self.assertIn("review follow-up + idempotent settlement live", page)
        self.assertIn("device-key protected service requests", page)
        self.assertIn("redacted review history live", page)
        self.assertIn("Request submission, follow-up, cancellation, and approval alone do not deduct credits", page)
        ...
        self.assertIn("the benefit transfer remains separate", page)
        self.assertIn("controlled HTTPS account UI", page)
        self.assertIn("read-only GCA balance verification", page)
        ...
        self.assertIn("Manual transfer; protected production evidence verification live", page)
        self.assertIn("support review queue", page)
        ...
        self.assertEqual(gates["status"], "public-release-gates-request-followup-live")
        ...
        self.assertEqual(gates["currentState"]["accountStatusRotationPacketVersion"], "gca_account_status_rotation_v1")
        self.assertTrue(gates["currentState"]["accountStatusRotationProductionLive"])
        self.assertEqual(gates["currentState"]["accountStatusRotationGraceMinutes"], 15)
        self.assertFalse(gates["currentState"]["accountStatusRotationReturnsAccessToken"])
        self.assertFalse(gates["currentState"]["accountStatusRotationChangesAccountOrLedgers"])
        ...
        self.assertTrue(gates["currentState"]["holdingHistoryVerificationProductionLive"])
        self.assertTrue(gates["currentState"]["onchainHoldingHistoryRequiredForApproval"])
        ...
        self.assertEqual(gates["currentState"]["latestPendingRouteReadinessCheckAt"], "2026-08-10T13:21:00Z")
        self.assertEqual(gates["currentState"]["latestPendingRoutePublicCheckAt"], "2026-08-10T13:23:22Z")
        self.assertEqual(gates["currentState"]["latestPendingRouteAdminCheckAt"], "2026-08-10T13:23:40Z")
        ...
        self.assertEqual(gates["currentState"]["workerVersionId"], "def4a0ea-fcbb-4d0e-a380-ba9656d7dc05")
        ...
        self.assertEqual(gates["currentState"]["baseScanTokenProfileLastCheckedDate"], "2026-08-12")
        self.assertEqual(gates["currentState"]["baseScanFinalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        self.assertEqual(gates["currentState"]["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
        ...
        for gate in {
            ...
            "service-request-reviewed-delivery",
        ...
        transfer_gate = next(item for item in gates["releaseGates"] if item["id"] == "member-benefit-transfer-evidence")
        service_delivery_gate = next(
            item for item in gates["releaseGates"] if item["id"] == "service-request-reviewed-delivery"
        )
        ...
        self.assertEqual(transfer_gate["status"], "live-token-protected-evidence-only")
        self.assertEqual(transfer_gate["packetVersion"], "gca_member_benefit_transfer_v1")
        self.assertEqual(transfer_gate["endpoint"], "/gca/member-benefit-transfers")
        self.assertEqual(transfer_gate["operatorTool"], "tools/record_cloudflare_member_benefit_transfer.py")
        self.assertEqual(transfer_gate["officialSourceWallet"], "0x5e8F84748612B913aAcC937492AC25dc5630E246")
        self.assertEqual(transfer_gate["exactTransferAmount"], "10000 GCA")
        self.assertEqual(service_delivery_gate["status"], "live-token-protected")
        self.assertEqual(service_delivery_gate["packetVersion"], "gca_service_request_review_v1")
        self.assertEqual(service_delivery_gate["endpoint"], "/gca/service-request-reviews")
        self.assertEqual(service_delivery_gate["operatorTool"], "tools/review_cloudflare_service_request.py")
        self.assertEqual(base_scan_gate["status"], "ready-for-owner-resubmission")
        self.assertEqual(base_scan_gate["finalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        self.assertEqual(base_scan_gate["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
        ...
        self.assertIn("The latest BaseScan owner submission package was generated on 2026-08-11T18:56:43Z, and the daily public status snapshot was refreshed on 2026-08-12T06:37:31Z.", gates["publicClaimBoundaries"]["safeClaims"])
    ...
    def test_verify_page_centralizes_official_identity_and_status(self):
        ...
        self.assertIn("Official channels", verify)
        self.assertIn("Canonical token", verify)
        self.assertIn("Live Read-Only Contract Identity", verify)
        self.assertIn("data-gca-contract-identity", verify)
        self.assertIn("data-contract-summary", verify)
        self.assertIn("data-contract-action", verify)
        self.assertIn('data-contract-field="network"', verify)
        self.assertIn('data-contract-field="supply"', verify)
        self.assertIn("assets/contract-identity.js?v=20260811-2", verify)
        self.assertIn('href="zh-wallet-verify.html"', verify)
        self.assertIn("does not connect a wallet, request permissions or signatures, or submit a transaction", verify)
        self.assertIn("official Base public RPC is rate-limited", verify)
        self.assertIn("Official market route", verify)
        self.assertIn("Anti-scam checklist", verify)
        ...
        self.assertIn("View BaseScan Source", verify)
        self.assertIn(X_URL, verify)
        self.assertIn("private keys", verify)
        self.assertIn("seed phrases", verify)
        self.assertIn("exchange API secrets", verify)
        self.assertIn("withdrawal permission", verify)
        self.assertNotIn("Current Review Status", verify)
        self.assertNotIn("Latest reviewer package", verify)
        self.assertNotIn("Returned 2026-05-23", verify)
        self.assertNotIn("Pending independent report", verify)
        self.assertNotIn("Listing Kit", verify)
    ...
    def test_market_snapshot_is_identity_pinned_and_read_only(self):
        script = (ROOT / "site" / "assets" / "market-snapshot.js").read_text()

        self.assertIn('const NETWORK = "base"', script)
        self.assertIn(OFFICIAL_POOL_ADDRESS.lower(), script)
        self.assertIn(MAINNET_ADDRESS.lower(), script)
        self.assertIn(BASE_USDT_ADDRESS.lower(), script)
        self.assertIn('const DEX_ID = "uniswap-v4-base"', script)
        self.assertIn('const API_VERSION = "20230302"', script)
    ...
    def test_contract_identity_check_is_exact_and_read_only(self):
        script = (ROOT / "site" / "assets" / "contract-identity.js").read_text()

        self.assertIn('const RPC_URL = "https://mainnet.base.org"', script)
        self.assertIn(MAINNET_ADDRESS, script)
        self.assertIn("const EXPECTED_CHAIN_ID = 8453n", script)
        self.assertIn('const EXPECTED_CHAIN_ID_HEX = "0x2105"', script)
        self.assertIn('const EXPECTED_NAME = "GCA"', script)
        self.assertIn('const EXPECTED_SYMBOL = "GCA"', script)
        self.assertIn("const EXPECTED_DECIMALS = 18n", script)
        self.assertIn("const EXPECTED_TOTAL_SUPPLY_RAW = 1000000000000000000000000000n", script)
        self.assertIn('method: "eth_chainId"', script)
        self.assertIn('method: "eth_getCode"', script)
        self.assertIn('method: "eth_call"', script)
        self.assertIn('data: "0x06fdde03"', script)
        self.assertIn('data: "0x95d89b41"', script)
        self.assertIn('data: "0x313ce567"', script)
        self.assertIn('data: "0x18160ddd"', script)
        self.assertIn('method: "POST"', script)
        self.assertIn('credentials: "omit"', script)
        self.assertIn('cache: "no-store"', script)
        self.assertIn('redirect: "error"', script)
        self.assertIn("MAX_RESPONSE_BYTES", script)
        self.assertIn("new TextDecoder", script)
        self.assertIn("BigInt", script)
        self.assertIn("Live identity matched", script)
        self.assertIn("正在从 Base Mainnet 读取固定合约", script)
        self.assertIn("实时身份一致", script)
        self.assertIn("检测到身份不一致", script)
        self.assertIn("实时只读检查暂时不可用", script)
        self.assertIn('source: "Base 公共 RPC"', script)
        self.assertIn('locale: "zh-CN"', script)
        self.assertIn("COPY.codePresent", script)
        self.assertNotIn("authorization:", script)
        self.assertNotIn("innerHTML", script)
        self.assertNotIn("localStorage", script)
        self.assertNotIn("sessionStorage", script)
        self.assertNotIn("document.cookie", script)
        self.assertNotIn("window.ethereum", script)
        self.assertNotIn("ethereum.request", script)
        self.assertNotIn("eth_sendTransaction", script)
        self.assertNotIn("eth_sendRawTransaction", script)
        self.assertNotIn("personal_sign", script)
        self.assertNotIn("eth_sign", script)
        self.assertNotIn("wallet_", script)

    def test_reserve_balance_check_is_exact_and_read_only(self):
        script = (ROOT / "site" / "assets" / "reserve-balance.js").read_text()

        self.assertIn('const RPC_URL = "https://mainnet.base.org"', script)
        self.assertIn(MAINNET_ADDRESS, script)
        self.assertIn(RESERVE_WALLET, script)
        self.assertIn("const EXPECTED_CHAIN_ID = 8453n", script)
        self.assertIn('const EXPECTED_CHAIN_ID_HEX = "0x2105"', script)
        self.assertIn("const EXPECTED_TOTAL_SUPPLY_RAW = 1000000000000000000000000000n", script)
        self.assertIn("const PUBLISHED_RESERVE_RAW = 600000000000000000000000000n", script)
        self.assertIn('method: "eth_chainId"', script)
        self.assertIn('method: "eth_blockNumber"', script)
        self.assertIn('method: "eth_call"', script)
        self.assertIn('data: "0x18160ddd"', script)
        self.assertIn("0x70a08231", script)
        self.assertIn("buildStateRequests(blockNumberHex)", script)
        self.assertIn('params: [{ to: CONTRACT_ADDRESS, data: "0x18160ddd" }, blockNumberHex]', script)
        self.assertIn("params: [{ to: CONTRACT_ADDRESS, data: BALANCE_OF_CALL_DATA }, blockNumberHex]", script)
        self.assertIn('method: "POST"', script)
        self.assertIn('credentials: "omit"', script)
        self.assertIn('cache: "no-store"', script)
        self.assertIn('redirect: "error"', script)
        self.assertIn("MAX_RESPONSE_BYTES", script)
        self.assertIn("BigInt", script)
        self.assertIn("data-gca-reserve-balance", script)
        self.assertIn("Read-only check passed", script)
        self.assertIn("只读核验成功", script)
        self.assertIn("Base 公共 RPC 有频率限制", script)
        self.assertNotIn("authorization:", script)
        self.assertNotIn("innerHTML", script)
        self.assertNotIn("localStorage", script)
        self.assertNotIn("sessionStorage", script)
        ...
        self.assertNotIn("eth_sign", script)
        self.assertNotIn("wallet_", script)
        self.assertNotIn("0x095ea7b3", script)
        self.assertNotIn("0xa9059cbb", script)
        self.assertNotIn("0x23b872dd", script)

    def test_chinese_wallet_verify_reuses_live_contract_identity(self):
        page = (ROOT / "site" / "zh-wallet-verify.html").read_text()

        self.assertIn("验证 GCA", page)
        self.assertIn("只读链上检查", page)
        self.assertIn("GCA 合约实时身份", page)
        self.assertIn("data-gca-contract-identity", page)
        self.assertIn("data-contract-summary", page)
        self.assertIn("data-contract-action", page)
        self.assertIn('data-contract-field="network"', page)
        self.assertIn('data-contract-field="supply"', page)
        self.assertIn("assets/contract-identity.js?v=20260811-2", page)
        self.assertIn("不会连接钱包、申请权限、请求签名或发送交易", page)
        self.assertIn("Base 公共 RPC 存在频率限制", page)
        self.assertIn("官方市场路线", page)
        self.assertIn("官方渠道", page)
        self.assertIn("防诈骗清单", page)
        self.assertNotIn("当前上线状态", page)
        self.assertNotIn("上线门槛", page)
        self.assertIn(MAINNET_ADDRESS, page)
        self.assertIn('href="verify.html"', page)
    ...
    def test_supply_page_explains_total_supply_and_reserve(self):
        ...
        self.assertIn("Live Read-Only Reserve Balance", supply)
        self.assertIn("data-gca-reserve-balance", supply)
        self.assertIn("data-reserve-summary", supply)
        self.assertIn('data-reserve-field="balance"', supply)
        self.assertIn('data-reserve-field="block"', supply)
        self.assertIn("assets/reserve-balance.js?v=20260811-1", supply)
        self.assertIn("does not prove a lock, vesting schedule, multisig", supply)
        self.assertIn("or circulating supply", supply)
    ...
    def test_status_page_centralizes_public_verification(self):
        ...
        self.assertIn('href="gca/member-access/"', product)
    ...
    def test_listing_readiness_page_gates_tracked_listing_submissions(self):
        ...
        self.assertIn("Snapshot refreshed 2026-08-12T06:37:31Z", page)
        self.assertIn("Final submission package generated 2026-08-11T18:56:43Z", page)
        self.assertIn("Domain Email Evidence Checklist", page)
        self.assertIn("domain-email-evidence.html", page)
    ...
    def test_review_queue_page_and_json_define_manual_review_contract(self):
        ...
        self.assertTrue(queue["currentState"]["localOperatorReviewQueueLive"])
        self.assertTrue(queue["currentState"]["localSupportReviewContinuityChainLive"])
        self.assertEqual(queue["currentState"]["localSupportReviewContinuityChainVersion"], "gca_support_review_audit_v1")
        self.assertFalse(queue["currentState"]["localSupportReviewContinuitySigned"])
        self.assertFalse(queue["currentState"]["localSupportReviewContinuityExternallyAnchored"])
        self.assertFalse(queue["currentState"]["localSupportReviewContinuityImmutable"])
        ...
        self.assertIn("auditSequence", queue["operatorControls"]["requiredAuditFields"])
        self.assertIn("auditRecordHash", queue["operatorControls"]["requiredAuditFields"])
        continuity = queue["localAuditContinuity"]
        self.assertEqual(continuity["status"], "live-local-operator-only")
        self.assertIn("verify_gca_support_review_audit.py", continuity["verificationCommand"])
        self.assertFalse(continuity["signed"])
        self.assertFalse(continuity["externallyAnchored"])
        self.assertFalse(continuity["immutable"])
        self.assertFalse(continuity["tailTruncationDetectableWithoutExternalHead"])
        self.assertFalse(continuity["fullLedgerReplacementDetectableWithoutExternalHead"])
        self.assertFalse(continuity["thirdPartyAudit"])
    ...
    def test_listing_kit_and_project_json_are_copyable(self):
        ...
        self.assertEqual(project["lastUpdated"], "2026-08-11")
        ...
        self.assertEqual(project["baseScanHandoff"]["finalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        self.assertEqual(project["baseScanHandoff"]["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
        ...
        self.assertIn("Token Rep remains Unknown", project["platformStatus"]["baseScanTokenProfileLastCheckedResult"])
        self.assertIn("2026-08-11T18:56:43Z", project["platformStatus"]["baseScanTokenProfileLastCheckedResult"])
        self.assertIn("2026-08-12T06:37:31Z", project["platformStatus"]["baseScanTokenProfileLastCheckedResult"])
        self.assertEqual(project["platformStatus"]["baseScanFinalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        ...
        self.assertEqual(project["platformStatus"]["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
        self.assertEqual(project["platformStatus"]["externalReviewStatusLastUpdated"], "2026-08-10")
        ...
        self.assertEqual(project["roadmap"]["currentStage"], PRODUCT_STAGE)
        ...
        self.assertEqual(project["weeklyGoChinaRadarIssue005"]["status"], "weekly-go-china-radar-issue-005-ready-for-review")
        self.assertEqual(project["weeklyGoChinaRadarIssue005"]["pageUrl"], RADAR_ISSUE_005_PAGE_URL)
        self.assertEqual(project["weeklyGoChinaRadarIssue005"]["url"], RADAR_ISSUE_005_URL)
        self.assertEqual(project["weeklyGoChinaRadarIssue005"]["issue"], "issue-005")
        self.assertEqual(project["weeklyGoChinaRadarIssue005"]["issueDate"], "2026-06-14")
        ...
        self.assertEqual(project["externalReviewStatus"]["lastUpdated"], "2026-08-10")
        self.assertEqual(project["externalReviewStatus"]["baseScanTokenProfile"], "ready-for-owner-resubmission")
        self.assertEqual(project["externalReviewStatus"]["baseScanTokenProfileLastCheckedDate"], "2026-08-12")
        self.assertIn("2026-08-11T18:56:43Z", project["externalReviewStatus"]["baseScanTokenProfileLastCheckedResult"])
        self.assertIn("2026-08-12T06:37:31Z", project["externalReviewStatus"]["baseScanTokenProfileLastCheckedResult"])
        ...
        self.assertEqual(project["externalReviewStatus"]["baseScanFinalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        self.assertEqual(project["externalReviewStatus"]["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
        ...
        self.assertEqual(project["contentCampaign"]["campaignCycleId"], "product-operations-002")
        self.assertEqual(project["contentCampaign"]["campaignWindow"], "2026-08-11 to 2026-09-07")
        self.assertEqual(project["contentCampaign"]["nextDraftTargetDate"], "2026-08-11")
        self.assertEqual(project["publishingDesk"]["campaignCycleId"], "product-operations-002")
        self.assertEqual(project["publishingDesk"]["nextPublishTargetDate"], "2026-08-11")
    ...
    def test_listing_readiness_json_gates_tracked_listing_submissions(self):
        ...
        self.assertEqual(readiness["lastUpdated"], "2026-08-10")
        ...
        self.assertEqual(checks["basescan-token-profile"]["lastCheckedDate"], "2026-08-12")
        self.assertEqual(checks["basescan-token-profile"]["finalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        self.assertEqual(checks["basescan-token-profile"]["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
        self.assertIn("Read-only public BaseScan check on 2026-08-12", checks["basescan-token-profile"]["evidence"])
        self.assertIn("prior 2026-05-23 information-insufficient return remains unresolved", checks["basescan-token-profile"]["evidence"])
        ...
        self.assertIn("2026-08-11T18:56:43Z", checks["basescan-token-profile"]["evidence"])
        self.assertIn("2026-08-12T06:37:31Z", checks["basescan-token-profile"]["evidence"])
    ...
    def test_external_reviews_page_and_json_centralize_review_status(self):
        ...
        self.assertIn("Read-only check 2026-08-12 still shows", page)
        self.assertIn("Token Rep: Unknown", page)
        ...
        self.assertEqual(reviews["lastUpdated"], "2026-08-10")
        ...
        self.assertEqual(reviews["reviews"]["baseScanTokenProfile"]["lastCheckedDate"], "2026-08-12")
        self.assertIn("Read-only public BaseScan check on 2026-08-12", reviews["reviews"]["baseScanTokenProfile"]["lastCheckedResult"])
        self.assertIn("Token Rep remains Unknown", reviews["reviews"]["baseScanTokenProfile"]["lastCheckedResult"])
        ...
        self.assertIn("2026-08-11T18:56:43Z", reviews["reviews"]["baseScanTokenProfile"]["lastCheckedResult"])
        self.assertIn("2026-08-12T06:37:31Z", reviews["reviews"]["baseScanTokenProfile"]["lastCheckedResult"])
        ...
        self.assertIn("The latest BaseScan final submission package was generated on 2026-08-11T18:56:43Z, and the daily public status snapshot confirms readyForBaseScanResubmission is true.", reviews["safePublicClaims"])
    ...
    def test_reviewer_kit_page_and_json_centralize_reviewer_evidence(self):
        ...
        self.assertIn("Latest reviewer package", page)
        self.assertIn("Final package generated 2026-08-11T18:56:43Z; daily status refreshed 2026-08-12T06:37:31Z", page)
        ...
        self.assertEqual(kit["lastUpdated"], "2026-08-10")
        self.assertEqual(kit["chainId"], 8453)
        self.assertEqual(kit["contractAddress"], MAINNET_ADDRESS)
        ...
        self.assertEqual(kit["externalReviewStatus"]["baseScanTokenProfileLastCheckedDate"], "2026-08-12")
        self.assertEqual(kit["externalReviewStatus"]["baseScanFinalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        self.assertEqual(kit["externalReviewStatus"]["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
        ...
        self.assertEqual(kit["baseScanResubmissionHandoff"]["finalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        self.assertEqual(kit["baseScanResubmissionHandoff"]["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
        ...
        self.assertIn("2026-08-11T18:56:43Z", " ".join(kit["reviewerUse"]["nextActions"]))
        self.assertIn("2026-08-12T06:37:31Z", " ".join(kit["reviewerUse"]["nextActions"]))
    ...
    def test_trust_center_page_and_json_centralize_public_verification(self):
        ...
        self.assertEqual(trust["lastUpdated"], "2026-08-10")
        ...
        self.assertEqual(trust["verificationSnapshot"]["baseScanTokenProfileLastCheckedDate"], "2026-08-12")
        self.assertEqual(trust["verificationSnapshot"]["baseScanFinalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        self.assertEqual(trust["verificationSnapshot"]["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
        ...
        self.assertEqual(trust["externalReviewStatus"]["baseScanTokenProfileLastCheckedDate"], "2026-08-12")
        self.assertEqual(trust["externalReviewStatus"]["baseScanFinalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        self.assertEqual(trust["externalReviewStatus"]["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
        ...
        self.assertIn("The latest BaseScan owner submission package was generated on 2026-08-11T18:56:43Z, and the daily public status snapshot was refreshed on 2026-08-12T06:37:31Z.", trust["safePublicClaims"])
    ...
    def test_market_quality_page_and_json_define_legitimate_growth_path(self):
        ...
        self.assertIn("Daily official-pool identity check live", page)
        self.assertIn('href="daily-status.html#marketHealth"', page)
        self.assertIn("Those values do not prove organic demand", page)
        ...
        self.assertEqual(quality["currentState"]["marketHealthMonitoring"], "live-read-only-daily-ops")
        self.assertEqual(quality["marketObservation"]["status"], "live-read-only")
        self.assertEqual(quality["marketObservation"]["provider"], "GeckoTerminal Public API")
        self.assertEqual(
            quality["marketObservation"]["dailyStatusPage"],
            f"{DAILY_STATUS_PAGE_URL}#marketHealth",
        )
        self.assertIn(
            "The snapshot is not an executable quote, trading signal, proof of organic demand, or liquidity guarantee.",
            quality["marketObservation"]["boundaries"],
        )
    ...
    def test_basescan_review_followup_is_actionable(self):
        ...
        self.assertIn("2026-08-12T06:37:31Z", followup)
    ...
    def test_basescan_form_values_are_copyable(self):
        ...
        self.assertIn("non-custodial risk tooling", values["description"])
        self.assertIn("ten browser-only public risk and research tools", values["description"])
        self.assertIn("do not fetch live market data, connect exchanges, or place orders", values["description"])
        ...
        self.assertEqual(values["lastCheckedDate"], "2026-08-12")
        self.assertIn("Read-only public BaseScan check on 2026-08-12", values["lastCheckedResult"])
        self.assertIn("Token Rep remains Unknown", values["lastCheckedResult"])
        ...
        self.assertIn("2026-08-11T18:56:43Z", values["lastCheckedResult"])
        self.assertIn("2026-08-12T06:37:31Z", values["lastCheckedResult"])
        ...
        self.assertEqual(values["baseScanFinalSubmissionPackageGeneratedAt"], "2026-08-11T18:56:43Z")
        ...
        self.assertEqual(values["dailyStatusGeneratedAt"], "2026-08-12T06:37:31Z")
    ...
    def test_basescan_resubmission_package_is_complete(self):
        ...
        self.assertIn("tools/sync_basescan_daily_status_references.py --check reports aligned with zero changed files", values["preflightCheck"]["readyRequires"])
        self.assertIn("tools/sync_basescan_final_package_references.py --check reports aligned with zero changed files after the canonical final package is built", values["preflightCheck"]["readyRequires"])
        ...
        self.assertIn("daily status reviewer reference alignment passes", values["submissionPackageBuilder"]["readyRequires"])
        self.assertIn("canonical final-package outputs automatically synchronize monitored handoff references", values["submissionPackageBuilder"]["readyRequires"])
    ...
    def test_external_review_followup_tracker_is_actionable_and_conservative(self):
        ...
        self.assertEqual(values["platforms"]["baseScan"]["lastCheckedDate"], "2026-08-12")
        ...
        self.assertIn("2026-08-11T18:56:43Z", values["platforms"]["baseScan"]["lastCheckedResult"])
        self.assertIn("2026-08-12T06:37:31Z", values["platforms"]["baseScan"]["lastCheckedResult"])
