...
The public access API contract includes live Workers + D1 routes for account intake, redacted device-key status, and wallet verification: `GET /gca/access-config`, `POST /gca/wallet-verifications`, `POST /gca/member-access`, `POST /gca/account-status`, plus token-protected member decisions at `GET/POST /gca/member-reviews` and append-only holding evidence reads at `GET /gca/holding-verifications`.
