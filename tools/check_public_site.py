...
def validate_verify(text: str) -> None:
    ...
    assert_contains(text, "Final package 2026-06-05T11:01:28Z; daily status 2026-06-05T10:29:19Z", label)
...
def validate_release_gates_json(text: str) -> None:
    ...
    if "The latest BaseScan owner submission package was generated on 2026-06-05T11:01:28Z, and the daily public status snapshot was refreshed on 2026-06-05T10:29:19Z." not in payload.get("publicClaimBoundaries", {}).get("safeClaims", []):
...
def validate_reviewer_kit_page(text: str) -> None:
    ...
    assert_contains(text, "Daily Status Queue", label)
    assert_contains(text, "Latest reviewer package", label)
    assert_contains(text, "Final package generated 2026-06-05T11:01:28Z; daily status refreshed 2026-06-05T10:29:19Z", label)
...
def validate_trust_json(text: str) -> None:
    ...
    if "The latest BaseScan owner submission package was generated on 2026-06-05T11:01:28Z, and the daily public status snapshot was refreshed on 2026-06-05T10:29:19Z." not in payload.get("safePublicClaims", []):
        raise SiteCheckError(f"{label}: missing latest BaseScan package safe claim")
    if "No third-party audit has been completed." not in payload.get("safePublicClaims", []):
        raise SiteCheckError(f"{label}: missing audit safe claim")
    if "security-vendor approval, permanent warning-free status, or cross-wallet warning removal before vendor/current wallet UI confirms it" not in payload.get("doNotClaim", []):
