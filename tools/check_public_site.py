...
def validate_zh_operations_page(text: str) -> None:
    ...
    for expected in (
        ...
        "服务请求队列和 Credit 使用记录已经在本地运营后端准备好",
        "Cloudflare Worker 路由仍需通过账号认证、D1、发布和待发布接口 smoke 检查后才能说正式上线",
        "Cloudflare Workers + D1 已上线",
        "gca-registration-api.gcagochina.workers.dev",
        ...
        "不能把 <code>/gca/service-requests</code> 或 <code>/gca/credit-usage</code> 说成已正式发布",
        "和会员审核怎么连接",
        "只读 GCA 余额验证",
        "30 天持有资料",
        "支持审核队列",
...
def validate_zh_access_page(text: str) -> None:
    ...
    for expected in (
        ...
        "可复制的会员审核资料包",
        "support@gcagochina.com",
        "服务请求队列和 Credit 使用记录目前是本地运营后端已准备",
...
def validate_zh_release_gates_page(text: str) -> None:
    ...
    for expected in (
        ...
        "10,000 GCA 会员权益",
        "符合条件写入账本，会员权益仍需人工审核",
        "本地运营已准备，Worker 路由待发布",
        "服务请求队列",
        "Credit 使用记录",
        "正式发布仍需要 Cloudflare 账号认证",
        "人工审核交付后的服务使用记录",
        "不能说公开 Worker 路由已上线",
        "不能说用户可自助消耗 credits",
        "本地运营后端可排队处理服务意向",
