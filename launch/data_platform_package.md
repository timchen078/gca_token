# GCA Data Platform Submission Package

This package prepares GCA information for token data and DEX discovery platforms. It is for accurate metadata submission only. It does not guarantee acceptance, ranking, trending placement, market data tracking, or price performance.
...
- BaseScan token profile: returned again by BaseScan as information-insufficient on 2026-05-23; remediation required before the next submission
...
- Social links: official Telegram is live at `https://t.me/gcagochinaofficial`; official X is live at `https://x.com/GCAAIGoChina`
...
- Minting: not supported after deployment
- Burning: not supported by custom token function
- Taxes: none
- Blacklist: none
- Admin controls: none
...
Public on-chain proofs are available at `https://gcagochina.com/onchain-proofs.html` and `https://gcagochina.com/onchain-proofs.json`. Use those public URLs when a reviewer asks for deployment transaction, source verification, reserve transfer transactions, official GCA/USDT pool, or functional swap evidence.
...
GCA, short for Go China Access, is a fixed-supply ERC-20 token on Base Mainnet with live account intake, read-only wallet verification, eligible member/credit ledgers, manually reviewed services, and ten browser-only risk tools; connected market-data and trading modules remain staged.

## Copyable Long Description

GCA is a fixed-supply ERC-20 token deployed on Base Mainnet. GCA currently stands for Go China Access, a concept-stage community direction focused on the Go China macro narrative, bilingual Web3 education, public market research, live account intake, read-only wallet verification, eligible member/credit ledger records, manually reviewed service requests, and ten live browser-only risk and research tools. The published GCA AI Quant Access layer connects those tools to liquidation replay reports, risk-warning reviews, realistic backtest review, ENTRY_READY review, position-size calculations, risk-control training, and a reserve-funded 10,000 GCA member benefit review for verified users who bought and continuously held at least 1,000,000 GCA for 30 days. Credits are deducted only after prior approval and completed delivery, at most once per request. Connected market-data and trading modules remain staged behind release gates. The contract created 1,000,000,000 GCA at deployment and includes no post-deployment minting, burning, taxes, blacklist, or admin controls. The official public pool is the Base Mainnet Uniswap v4 GCA/USDT pool, and the project has a disclosed owner-held reserve wallet. The project is early-stage. Third-party audit quote requests were submitted and then deferred by owner decision; the project should not be described as externally audited, price-backed, yield-bearing, or backed by guaranteed liquidity.
...
The public product spec is published at `https://gcagochina.com/product.html` and `https://gcagochina.com/product.json`. It defines GCA AI Quant Access as a live account, browser-tool, and manually reviewed service layer with China Narrative Radar, Weekly Go China Radar, Liquidation Replay, Risk Warning Review, Backtest Lab, ENTRY_READY Review, Position Size Calculator, and GCA Member Workspace. Connected market-data and trading modules remain gated behind product release controls.
...
The public credits catalog is published at `https://gcagochina.com/credits.html` and `https://gcagochina.com/credits.json`. It is a live manual-review service catalog for 100 GCA AI Quant Access credits and GCA Member workflows, including Liquidation Replay, Risk Warning Review, Backtest Lab, ENTRY_READY Review, Position Size Calculator, Portfolio Risk Map, Risk-Control Training, Member Research Notes, and Support Review Queue. Eligible accounts can submit requests and read redacted status; credits settle only after prior approval and completed delivery, at most once per request. Token-benefit transfers remain separately controlled by support review and manual reserve-wallet processing.

The product release gates are published at `https://gcagochina.com/release-gates.html` and `https://gcagochina.com/release-gates.json`. They show the controlled HTTPS account UI, read-only GCA balance verification, credit/member ledger records, and manually reviewed service delivery as live, while member-benefit manual processing, connected market-data integration, risk-control review, and simulation or testnet first remain required before any broader trading workflow.
...
4. Include only the official X profile published on the website: `https://x.com/GCAAIGoChina`.

### CoinGecko

Status: package prepared, but listing readiness is weak until there is stronger public activity, social presence, and meaningful exchange volume.

CoinGecko states that listing is free, does not guarantee acceptance, and requires accurate information. Its criteria include a working project-owned website, a working block explorer, listing on at least one active exchange integrated with CoinGecko, and clear circulating supply communication.

Recommended action:

1. Do not submit duplicate requests.
2. Submit only after the project has stronger public presence and enough trading/market activity.
3. If submitting early, frame it as an early concept-stage token with starter liquidity and no external audit.

### CoinMarketCap

Status: package prepared, but tracked listing readiness is weak because current liquidity and volume are very small.

CoinMarketCap states that its online submission form is the only path for listing or update requests. Its tracked listing guidelines include a functional website, block explorer, public active trading with material volume on at least one tracked exchange, and a project representative contact. Its house rules warn against duplicate requests, bribery, spam, fake figures, and repeated status chasing.

Recommended action:

1. Do not submit repeatedly.
2. Do not use any third-party service claiming guaranteed CMC listing.
3. Wait for stronger liquidity, organic holders, public activity, and BaseScan profile publication before requesting a tracked listing.

## Official Source References

- DEX Screener token listing docs: `https://docs.dexscreener.com/token-listing`
- GeckoTerminal token information update guide: `https://support.coingecko.com/hc/en-us/articles/22612245806745-How-do-I-update-token-information-on-GeckoTerminal`
- CoinGecko methodology and listing criteria: `https://www.coingecko.com/en/methodology`
- CoinGecko self-serve request guide: `https://support.coingecko.com/hc/en-us/articles/33084534107289-Guide-to-the-CoinGecko-Self-Serve-Request-Form`
