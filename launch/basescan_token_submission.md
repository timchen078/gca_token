# BaseScan Token Information Submission

BaseScan token updates must be submitted through the official BaseScan token update flow after the contract source has been verified and address ownership has been claimed by the deployer wallet.

## Official Submission Links

- Guidelines: `https://info.basescan.org/how-to-update-token-info/`
- Token update form: `https://basescan.org/tokenupdate/`
- Address ownership guide: `https://info.basescan.org/verifyaddress/`
...
- Daily public status refreshed: `2026-08-12T06:37:31Z`

## Neutral Project Description

GCA is a fixed-supply ERC-20 token deployed on Base Mainnet and a concept-stage Go China Access community focused on the Go China macro narrative, bilingual Web3 education, public market research, and non-custodial risk tooling. The project publishes live account and member intake, ten browser-only public risk and research tools, and device-key service requests that enter a manual review and delivery workflow. Credits are deducted only after prior approval and completed delivery, at most once per request. The browser tools do not fetch live market data, connect exchanges, or place orders; connected market-data and trading modules remain staged behind release gates. The contract creates 1,000,000,000 GCA at deployment and includes no post-deployment minting, burning, taxes, blacklist, or admin controls.
...
- Email note: project-domain mailbox, publicly listed on the official website and used for the next clean BaseScan resubmission.

## Fields That Still Need Owner Action

- Submit one clean BaseScan token profile update from `support@gcagochina.com` after the final preflight passes.
- Wait for BaseScan review or an email reply to `support@gcagochina.com`.

## Submission Rules

- Submit only once after all links are public and working.
...
Submitted from the owner's browser session on 2026-05-09 with the earlier owner email, returned as information-insufficient on 2026-05-13, resubmitted on 2026-05-13, and returned again as information-insufficient on 2026-05-23. The current next submission should use `support@gcagochina.com`, GCA team responsibility evidence, the Project Profile BaseScan reviewer map, the public domain-email evidence checklist, BaseScan Handoff copy blocks generated at `2026-08-11T18:56:43Z`, the daily public status snapshot refreshed at `2026-08-12T06:37:31Z`, Telegram, X, project profile, and the official GCA/USDT route. The public token profile is not complete until BaseScan accepts and publishes the submitted update.
