...
- Local-only member backend prepared at `tools/gca_member_backend.py`; it serves `site/`, enables same-origin localhost submission, verifies GCA balances through read-only Base Mainnet `eth_call`, exposes the local operator console at `http://127.0.0.1:8787/operator.html`, and writes local JSONL pre-registration, wallet verification, 100 credits, GCA Member, and review records under `.gca_access_data/`.
