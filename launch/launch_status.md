...
- BaseScan token profile remediation package is published; Tim Chen public professional profile evidence is now published, and the 2026-05-27 DNS snapshot shows MX/SPF/DMARC missing with DKIM selector required, so the next submission should wait until a project-domain email is ready.
...
- Use the domain email setup plan at `https://gcagochina.com/domain-email.html`, fix the 2026-05-27 DNS snapshot blockers at `https://gcagochina.com/domain-email.html#snapshotTitle` (MX/SPF/DMARC missing and DKIM selector required), create and publish a working `gcagochina.com` domain email, run `python3 tools/check_domain_email_dns.py --domain gcagochina.com --mailbox support --dkim-selector <provider-selector> --json`, and archive the activation evidence packet before the next BaseScan submission.
